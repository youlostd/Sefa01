...
#define _DETOUR_LOG_TO_VECTOR_

vector<string>		m_vPacketLogExtended;
void AppendPacketLogExt(string data)
{
	m_vPacketLogExtended.push_back(data);
	if (m_vPacketLogExtended.size() > EXT_PACKET_ERROR_DUMP)
		m_vPacketLogExtended.erase(m_vPacketLogExtended.begin());
}

void ExtendedPacketErrorDump()
{
	if (!(_access_s("syserr_reporter.exe", 0) == 0))
	{
		TraceError("syserr_reporter.exe doesnt exists.");
		return;
	}
	
	FILE* pFile = fopen("packet_errors.log", "w+");
	for (auto const& value : m_vPacketLogExtended)
		fprintf(pFile, value.c_str());
	fclose(pFile);

	ShellExecuteA(NULL, NULL, "syserr_reporter.exe", "game", NULL, SW_SHOW);
}
#endif

void LogFile(const char * c_szMsg)
{
#ifdef _DETOUR_LOG_TO_VECTOR_
	AppendPacketLogExt(c_szMsg);
#else
