...
bool CMapOutdoor::Update(float fX, float fY, float fZ)
{
	...
	RefreshShownTrees(m_bForceRefreshTree);
	if (m_bForceRefreshTree)
		m_bForceRefreshTree = false;
#endif

#ifdef ENABLE_GRAVEL_HIDE_SYSTEM
	RefreshShownGravel(m_bForceRefreshGravel);
	if (m_bForceRefreshGravel)
		m_bForceRefreshGravel = false;
#endif
...
#endif
#ifndef ENABLE_HIDE_ONMOVE_SHADOWS
