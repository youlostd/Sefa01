...
#define CONFIG_NAME_TREE "perf_tree_range"
#endif

#ifdef ENABLE_GRAVEL_HIDE_SYSTEM
#define CONFIG_NAME_GRAVEL "perf_gravel_range"
#endif
...
#ifdef ENABLE_TREE_HIDE_SYSTEM
	float fTreeShowRange = CPythonConfig__GetOptionFloat(CONFIG_NAME_TREE, 0.0f);
#endif

#ifdef ENABLE_GRAVEL_HIDE_SYSTEM
	float fGravelShowRange = CPythonConfig__GetOptionFloat(CONFIG_NAME_GRAVEL, 0.0f);
#endif
...
#ifdef ENABLE_TREE_HIDE_SYSTEM
			pArea->ShowTrees(fTreeShowRange);
#endif

#ifdef ENABLE_GRAVEL_HIDE_SYSTEM
			if (m_bForceGravelHide)
				pArea->HideGravel();
			else
				pArea->ShowGravel(fGravelShowRange);
#endif
		}
	}
}
...
#ifdef ENABLE_TREE_HIDE_SYSTEM
void CMapOutdoor::RefreshShownTrees(bool bForce)
{
	static const DWORD s_dwTimeoutTime = 500;
	static DWORD s_dwLastTime = 0;
	DWORD dwTimeNow = ELTimer_GetMSec();

	if (s_dwLastTime && !bForce)
	{
		if (dwTimeNow > s_dwLastTime && dwTimeNow - s_dwLastTime < s_dwTimeoutTime)
			return;
	}

	s_dwLastTime = dwTimeNow;

	float fTreeShowRange = CPythonConfig__GetOptionFloat(CONFIG_NAME_TREE, 0.0f);
	for (int i = 0; i < AROUND_AREA_NUM; ++i)
	{
		if (m_pArea[i])
			m_pArea[i]->ShowTrees(fTreeShowRange);
	}
}
#endif

#ifdef ENABLE_GRAVEL_HIDE_SYSTEM
void CMapOutdoor::RefreshGravel()
{
	float fGravelShowRange = 0.0f;
	if (!m_bForceGravelHide)
		fGravelShowRange = CPythonConfig__GetOptionFloat(CONFIG_NAME_GRAVEL, 0.0f);
	...
	for (int i = 0; i < AROUND_AREA_NUM; ++i)
	{
		if (m_pArea[i])
		{
			if (m_bForceGravelHide)
				...
				m_pArea[i]->ShowGravel(fGravelShowRange);
		}
	}
}

void CMapOutdoor::RefreshShownGravel(bool bForce)
{
	if (m_bForceGravelHide)
		return;
...
bool CMapOutdoor::LoadSetting(const char * c_szFileName)
{
	...
	{
		int type = atoi(stTokenVectorMap["watertype"][0].c_str());
		SetWaterType(type);
	}
