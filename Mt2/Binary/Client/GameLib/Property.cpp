...
const char * CProperty::GetInternFileName()
{
	if (m_stInternFileName.empty())
	{
		int iPos = m_stFileName.find("property");
		if (iPos != std::string::npos)
			m_stInternFileName = m_stFileName.c_str() + iPos;
		else
			m_stInternFileName = m_stFileName;
	}

	return m_stInternFileName.c_str();
}
#endif
