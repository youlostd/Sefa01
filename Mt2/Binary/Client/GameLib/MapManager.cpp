...
void CMapManager::__LoadMapInfoVector()
{
	...
	for (UINT uLineIndex=0; uLineIndex<textFileLoader.GetLineCount(); ++uLineIndex)
	{
		...
		std::string mapName = std::string(szMapName) + "/000000/attr.atr";
		if (!CEterPackManager::Instance().isExistInPack(mapName.c_str()))
		{
			mapName = mapName.substr(mapName.find("/") + 1);
			if (!CEterPackManager::Instance().isExistInPack(mapName.c_str()))
				if (test_server)
					TraceError("Could not find %s", mapName.c_str());
			else
			{
				mapName = std::string(szMapName);
				mapName = mapName.substr(mapName.find("/") + 1);
				strcpy(szMapName, mapName.c_str());
			}
		}
