...
bool CRaceMotionData::LoadMotionData(const char * c_szFileName)
{
	...
	if (!pkTextFileLoader)
	{
		TraceError("Invalid TextFileLoader[%s]", c_szFileName);
		return false;
	}
	...
	{
		TraceError("TextFileLoader[%s] empty", c_szFileName);
		return false;
	}
	...
	for (DWORD i = 0; i < rkTextFileLoader.GetChildNodeCount(); ++i)
	{
		...
		if (0 == strNodeName.compare("comboinputdata"))
		{
			...
			{
				TraceError("No preinputtime for comboinputdata [child %d]", i);
				return false;
			}
			if (!rkTextFileLoader.GetTokenFloat("directinputtime", &m_ComboInputData.fNextComboTime))
			{
				TraceError("No preinputtime for directinputtime [child %d]", i);
				return false;
			}
			if (!rkTextFileLoader.GetTokenFloat("inputlimittime", &m_ComboInputData.fInputEndTime))
			{
				TraceError("No preinputtime for inputlimittime [child %d]", i);
				return false;
			}
		}
		else if (0 == strNodeName.compare("attackingdata"))
		{
			...
			{
				TraceError("Cannot load motion attack data for attackingdata [child %d]", i);
				return false;
			}
		}
		else if (0 == strNodeName.compare("loopdata"))
		{
			...
			{
				TraceError("No loopstarttime for loopdata [child %d]", i);
				return false;
			}
			if (!rkTextFileLoader.GetTokenFloat("loopendtime", &m_fLoopEndTime))
			{
				TraceError("No loopendtime for loopdata [child %d]", i);
				return false;
			}
		}
		else if (0 == strNodeName.compare("motioneventdata"))
		{
			...
			for (DWORD j = 0; j < m_MotionEventDataVector.size(); ++j)
			{
				...
				{
					TraceError("No event for child[%d] for motioneventdata [child %d]", j, i);
					return false;
				}
				...
				{
					TraceError("No motioneventtype for child[%d] for motioneventdata [child %d]", j, i);
					return false;
				}
...
#endif
					default:
						TraceError("Strange event type [%d]", iType);
				...
				{
					TraceError("No startingtime for child[%d] for motioneventdata [child %d]", j, i);
					return false;
				}
