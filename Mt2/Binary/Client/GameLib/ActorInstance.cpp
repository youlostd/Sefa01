...
#include "RaceData.h"
#define CHARACTER_NAME_MAX_LEN 35
...
void CActorInstance::OnUpdate()
{
	...
	DWORD t4=ELTimer_GetMSec();
	{
		...
		if (t4-t1)
		{
			fprintf(fp, "AIU2.Total %d (Time %.3f)\n", 
...
void CActorInstance::SetMoveThroughMonster(bool bCanMoveThroughMonster)
{
	m_canMoveThroughMonster=bCanMoveThroughMonster;
}
...
bool CActorInstance::IsFakeBuff()
{
	if (TYPE_FAKEBUFF == m_eActorType)
		return true;
...
{
	if (TYPE_ENEMY==m_eActorType || TYPE_BOSS == m_eActorType)
		return true;
...
void CActorInstance::MountHorse(CActorInstance * pkHorse)
{
	...
	if (m_pkHorse)
	{
		...
		m_pkHorse->m_isMounted = true;
		m_pkHorse->RefreshActorInstance();
	}
}
