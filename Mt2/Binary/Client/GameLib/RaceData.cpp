...
void CRaceData::LoadMotions()
{
	...
	if (CreateMotionModeIterator(itor))
	{
		WORD wMotionMode;
		CRaceData::TMotionModeData * pMotionModeData;
		...
		do
		{
			wMotionMode = itor->first;
			pMotionModeData = itor->second;

			itorMotion = pMotionModeData->MotionVectorMap.begin();
			for (; itorMotion != pMotionModeData->MotionVectorMap.end(); ++itorMotion)
			{
				wMotionIndex = itorMotion->first;
				...
				{
					dwMotionKey = MAKE_RANDOM_MOTION_KEY(wMotionMode, wMotionIndex, i);
					CGraphicThingInstance::RegisterMotionThing(m_dwRaceIndex, dwMotionKey, it->pMotion);
				}
			}
		} while (NextMotionModeIterator(itor));
	}
}
