...
CRaceData* CRaceManager::__LoadRaceData(DWORD dwRaceIndex)
{
	...
	for (int i = 0; i < vec_stFullPathName.size(); i++)
	{
		...
		if (!pRaceData->LoadRaceData(stMSMFileName.c_str()))
		{
			...
			{
				//TraceError("CRaceManager::RegisterRacePath : RACE[%u] LOAD MSMFILE[%s] ERROR. Will Find Another Path (current Path %s).", dwRaceIndex, stMSMFileName.c_str(), vec_stFullPathName[i].c_str());
				continue;
			}
...
BOOL CRaceManager::GetRaceDataPointer(DWORD dwRaceIndex, CRaceData ** ppRaceData)
{
	...
	if (m_RaceDataMap.end() == itor)
	{
		...
		if (dwRaceIndex != 34000 && dwRaceIndex != 34038)
...
void CRaceManager::SetRaceSpecular(int iVnum, float fSpecular)
{
	m_kMap_dwRaceKey_stRaceSpecular.insert(std::map<DWORD, float>::value_type(iVnum, fSpecular));
}

float CRaceManager::GetRaceSpecular(int iVnum)
{
	std::map<DWORD, float>::iterator it = m_kMap_dwRaceKey_stRaceSpecular.find(iVnum);
	if (m_kMap_dwRaceKey_stRaceSpecular.end() == it)
		return 0.0f;
