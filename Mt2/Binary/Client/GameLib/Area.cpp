...
#ifdef ENABLE_GRAVEL_HIDE_SYSTEM
const char* c_pszGravelPathList[] = {
	"property/Shyeonline/Trees/mde",
	"property/Shyeonline/Trees/mde2",
	"property/wiskypeak/grass",
	"Property/PlantUpdates",
	NULL,
};
#endif

#ifdef ENABLE_TREE_HIDE_SYSTEM
void CArea::HideTrees()
{
	for (int i = 0; i < m_ObjectInstanceVector.size(); ++i)
	{
		TObjectInstance* pObjectInstance = m_ObjectInstanceVector[i];
		if (prt::PROPERTY_TYPE_TREE == pObjectInstance->dwType)
		{
			if (pObjectInstance->pTree && pObjectInstance->pTree->IsManualShowTree())
			{
				pObjectInstance->pTree->ManualHideTree();
			}
		}
	}
}

void CArea::ShowTrees(float fRangeLimit)
{
	...
	for (int i = 0; i < m_ObjectInstanceVector.size(); ++i)
	{
		...
		if (prt::PROPERTY_TYPE_TREE == pObjectInstance->dwType)
		{
			...
			{
				pObjectInstance->pTree->ManualShowTree();

				if (fRangeLimit)
				{
					const float* c_afTreePos = pObjectInstance->pTree->GetPosition();

					float fDist = DISTANCE_APPROX(kMainPos.x - c_afTreePos[0], kMainPos.y - (-c_afTreePos[1]));
					if (fDist > fRangeLimit)
						pObjectInstance->pTree->ManualHideTree();
				}
			}
		}
...
#ifdef ENABLE_GRAVEL_HIDE_SYSTEM
	pObjectInstance->bIsGravel = c_pData->bIsGravel;
#endif
...
void CArea::__SetObjectInstance_SetTree(TObjectInstance * pObjectInstance, const TObjectData * c_pData, CProperty * pProperty)
{
	...
	//TraceError("LoadTree AT %dx%d", (int)c_pData->Position.x / 100, (int)-c_pData->Position.y / 100);
...
inline bool str_startswith(const char* c_pszMainStr, const char** c_pszStartsWith)
{
	int i = 0;
	while (c_pszStartsWith[i] != NULL)
	{
		if (!strnicmp(c_pszMainStr, c_pszStartsWith[i], strlen(c_pszStartsWith[i])))
			return true;
...
bool CArea::__Load_LoadObject(const char * c_szFileName)
{
	...
	for (DWORD i = 0; i < dwCount; ++i)
	{
		...
		char file_name[256];
		char ext_name[100];
		_splitpath_s(pProperty->GetInternFileName(), drive_name, dir_name, file_name, ext_name);
		ObjectData.bIsGravel = str_startswith(pProperty->GetInternFileName(), c_pszGravelPathList) ||
			(std::string(file_name).find("grass") != std::string::npos && !strcmp(ext_name, ".pre"));
#endif
...
#ifdef ENABLE_GRAVEL_HIDE_SYSTEM
void CArea::HideGravel()
{
	for (int i = 0; i < m_ObjectInstanceVector.size(); ++i)
	{
		TObjectInstance* pObjectInstance = m_ObjectInstanceVector[i];
		if (pObjectInstance->bIsGravel)
		{
			if (pObjectInstance->dwEffectInstanceIndex != 0xffffffff)
			{
				TEffectInstanceIterator f = m_EffectInstanceMap.find(pObjectInstance->dwEffectInstanceIndex);
			...
			if (pObjectInstance->pThingInstance)
			{
				pObjectInstance->pThingInstance->HideManual();
			}

			if (pObjectInstance->pAmbienceInstance)
			{
				pObjectInstance->pAmbienceInstance->HideManual();
			}
...
void CArea::ShowGravel(float fRangeLimit)
{
	CInstanceBase* pInstMain = CPythonCharacterManager::Instance().GetMainInstancePtr();
	TPixelPosition kMainPos;
	...
	for (int i = 0; i < m_ObjectInstanceVector.size(); ++i)
	{
		...
		if (pObjectInstance->bIsGravel)
		{
			if (pObjectInstance->dwEffectInstanceIndex != 0xffffffff)
			{
				...
				{
					CEffectInstance * pEffectInstance = f->second;
					pEffectInstance->ShowManual();

					if (fRangeLimit)
					{
						const D3DXMATRIX& rkEffMat = pEffectInstance->GetGlobalMatrix();
			...
			if (pObjectInstance->pThingInstance)
			{
				pObjectInstance->pThingInstance->ShowManual();
			}

			if (pObjectInstance->pAmbienceInstance)
			{
				pObjectInstance->pAmbienceInstance->ShowManual();
			}
