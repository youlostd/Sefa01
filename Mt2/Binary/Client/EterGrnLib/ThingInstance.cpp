...
std::unordered_map<DWORD, std::map<DWORD, CGraphicThing::TRef *>>	CGraphicThingInstance::m_roMotionThingMap;
...
{
	std::map<DWORD, CGraphicThing::TRef *>::iterator itor = m_roMotionThingMap[m_dwMotionID].find(dwMotionKey);

	if (m_roMotionThingMap[m_dwMotionID].end() == itor)
		return false;
...
bool CGraphicThingInstance::GetMotionThingPointer(DWORD dwKey, CGraphicThing ** ppMotion)
{
	...
	*ppMotion = m_roMotionThingMap[m_dwMotionID][dwKey]->GetPointer();
	return true;
}
...
void CGraphicThingInstance::RegisterMotionThing(DWORD motionID, DWORD dwMotionKey, CGraphicThing* pMotionThing)
{
	CGraphicThing::TRef * pMotionRef = new CGraphicThing::TRef;
	if (!m_roMotionThingMap[motionID].insert(std::map<DWORD, CGraphicThing::TRef *>::value_type(dwMotionKey, pMotionRef)).second)
		delete pMotionRef;
	else
...
void CGraphicThingInstance::DeformNoSkin()
{
	...
	for (std::vector<CGrannyLODController*>::size_type i = 0; i != m_LODControllerVector.size(); i++)
	{
		...
		if (pkLOD->isModelInstance()) {
			if (i == 5 && m_bIsAcceTransform) { //AcceTransform
