...
PyObject * eventIsLocalWait(PyObject * poSelf, PyObject * poArgs)
{
	int iIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iIndex))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonEventManager::Instance().IsWait(iIndex, true) == true ? 1 : 0);
}
...
PyObject * eventSetFontColor(PyObject* poSelf, PyObject* poArgs)
{
	...
	CPythonEventManager::Instance().SetFontColor(iIndex, r, g, b, a);
	return Py_BuildNone();
}
...
void initEvent()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "IsLocalWait",				eventIsLocalWait,					METH_VARARGS },
