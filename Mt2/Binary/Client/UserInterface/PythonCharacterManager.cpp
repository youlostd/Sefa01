...
void CPythonCharacterManager::Update()
{
...
#ifdef __PERFORMANCE_CHECKER__
	{
		...
		if (t6-t1>1)
		{
			fprintf(fp, "CU.Total %d (Time %.3f, Alive %d, Dead %d)\n", 
				t6-t1, ELTimer_GetMSec()/1000.0f,
...
void CPythonCharacterManager::UpdateTransform()
{
...
#ifdef __PERFORMANCE_CHECKER__
	{
		...
		if (t4-t1)
		{
			fprintf(fp, "CUT.Total %d (Time %.3f, Alive %d, Dead %d)\n", 
...
void CPythonCharacterManager::UpdateDeleting()
{
	...
	for (; itor != m_kDeadInstList.end();)
	{
		...
		if (m_kDeadInstList.size() > 10)
		{
			if (!pInstance)
				continue;

			if (GetMainInstancePtr())
			{
				int nDistance = int(pInstance->NEW_GetDistanceFromDestInstance(*GetMainInstancePtr()));
				if (nDistance > CHAR_STAGE_VIEW_BOUND + 10)
				{
