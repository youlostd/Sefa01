...
PyObject * systemSetHideNPC(PyObject * poSelf, PyObject * poArgs)
{
	int category = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &category))
		return Py_BuildException();

	int show = 0;
	if (!PyTuple_GetInteger(poArgs, 1, &show))
		return Py_BuildException();

	CPythonSystem::Instance().SetHideNPC(category, show);
	return Py_BuildNone();
}

PyObject * systemIsHiddenNPC(PyObject * poSelf, PyObject * poArgs)
{
	int category = 0;
	if (!PyTuple_GetInteger(poArgs, 0, &category))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonSystem::Instance().IsHiddenNPC(category));
}
#endif
...
#ifdef HIDE_NPC_OPTION
		{ "SetHideNPC",					systemSetHideNPC,				METH_VARARGS },
		{ "IsHiddenNPC",				systemIsHiddenNPC,				METH_VARARGS },
#endif
