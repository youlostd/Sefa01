...
#ifdef ENABLE_ALL_PICKUP
	if (CPythonConfig::Instance().GetInteger(CPythonConfig::CLASS_OPTION, "pickup_all_fast", 1) == 1 || GetAsyncKeyState(VK_LSHIFT) & 0x8001 || GetAsyncKeyState(VK_LCONTROL) & 0x8001)
	{
		std::vector<DWORD> vecItemID;
		if (rkItem.GetCloseItemList(kPPosMain, &vecItemID, __GetPickableDistance()))
		{
			for (DWORD dwItemID : vecItemID)
				SendClickItemPacket(dwItemID, true);
