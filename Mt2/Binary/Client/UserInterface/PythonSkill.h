...
class CPythonSkill : public CSingleton<CPythonSkill>
{
	public:
		...
		enum ESkillTableTokenType
		{
			...
			TABLE_TOKEN_TYPE_AFFECT_FLAG,//15
#ifdef ENABLE_LEGENDARY_SKILL
			TABLE_TOKEN_TYPE_AFFECT_FLAG_LEGENDARY,
#endif
...
#ifdef ENABLE_LEGENDARY_SKILL
			TABLE_TOKEN_TYPE_AFFECT_FLAG2_LEGENDARY,
#endif
			TABLE_TOKEN_TYPE_POINT_ON3,//20
			TABLE_TOKEN_TYPE_POINT_POLY3,
			TABLE_TOKEN_TYPE_DURATION_POLY3,// 22
			// TABLE_TOKEN_TYPE_AFFECT_FLAG_PERFECT,
			TABLE_TOKEN_TYPE_GRAND_MASTER_ADD_SP_COST_POLY,
			TABLE_TOKEN_TYPE_PREREQUISITE_SKILL_VNUM,
			TABLE_TOKEN_TYPE_PREREQUISITE_SKILL_LEVEL,
			TABLE_TOKEN_TYPE_SKILL_TYPE, // 26
		...
		enum
		{
			...
			SKILL_NEED_WEAPON_QUIVER			= (1 << CItemData::WEAPON_QUIVER),
		...
		enum
		{
			...
			SKILL_GRADE_COUNT = 3,
#endif
			SKILL_EFFECT_COUNT = 5,	//스킬은 초,중,고급 이외에 마스터일때 이펙트 추가.
		...
		typedef struct SSkillData
		{
			...
			float ProcessFormula(CPoly * pPoly, float fSkillLevel = 0.0f, int iMinMaxType = VALUE_TYPE_FREE, BYTE realLevel = 0);
			const char * GetAffectDescription(DWORD dwIndex, float fSkillLevel, BYTE realLevel);
