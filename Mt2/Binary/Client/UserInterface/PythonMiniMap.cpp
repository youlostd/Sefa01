...
void CPythonMiniMap::Update(float fCenterX, float fCenterY)
{
	...
	for(i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd(); ++i)
	{
...
#ifdef HIDE_MINIMAP_DEAD_ENTITIES
		...
		else if (pkInstEach->IsBoss()) // must be before IsEnemy since all boss == enemy but all enemy != boss
		{
			aMarkPosition.m_fX = (m_fWidth - (float)m_WhiteMark.GetWidth()) / 2.0f + fDistanceFromCenterX + m_fScreenX;
			aMarkPosition.m_fY = (m_fHeight - (float)m_WhiteMark.GetHeight()) / 2.0f + fDistanceFromCenterY + m_fScreenY;
...
void CPythonMiniMap::__LoadAtlasMarkInfo()
{
	...
	_snprintf(szAtlasMarkInfoFileName, sizeof(szAtlasMarkInfoFileName), "%s/map/%s_point.txt", CLocaleManager::instance().GetLocaleBasePath().c_str(), rkMap.GetName().c_str());
	// END_OF_LOCALE
...
bool CPythonMiniMap::LoadAtlas()
{
	...
	string stMapName;
	if (rkMap.GetName().find('/') != std::string::npos)
		stMapName = (rkMap.GetName().c_str() + rkMap.GetName().find_last_of('/') + 1);
	else
		stMapName = rkMap.GetName();
...
void CPythonMiniMap::LoadAtlasData()
{
	CPythonBackground& rkBG = CPythonBackground::Instance();
	if (!rkBG.IsMapOutdoor())
		return;
...
void CPythonMiniMap::RenderAtlas(float fScreenX, float fScreenY)
{
	...
	std::vector<DWORD> vec_dwRemoveID;
	...
	for (; m_AtlasMarkInfoVectorIterator != m_AtlasWayPointInfoVector.end(); ++m_AtlasMarkInfoVectorIterator)
	{
		...
		{
			__RenderWayPointMark(rAtlasMarkInfo.m_fScreenX, rAtlasMarkInfo.m_fScreenY, rAtlasMarkInfo);

			if (!rAtlasMarkInfo.m_bRepeat)
			{
				if (rAtlasMarkInfo.m_iRenderTime >= WAYPOINT_IMAGE_COUNT * 2)
					vec_dwRemoveID.push_back(rAtlasMarkInfo.m_dwID);
			}
		}
	}
	for (int i = 0; i < vec_dwRemoveID.size(); ++i)
		RemoveWayPoint(vec_dwRemoveID[i]);
...
//////////////////////////////////////////////////////////////////////////
// WayPoint
void CPythonMiniMap::AddWayPoint(BYTE byType, DWORD dwID, float fX, float fY, std::string strText, DWORD dwChrVID, bool bRepeat)
{
	m_AtlasMarkInfoVectorIterator = m_AtlasWayPointInfoVector.begin();
	while (m_AtlasMarkInfoVectorIterator != m_AtlasWayPointInfoVector.end())
	{
	...
	aAtlasMarkInfo.m_bRepeat = bRepeat;
...
void CPythonMiniMap::__RenderWayPointMark(int ixCenter, int iyCenter, TAtlasMarkInfo& rkInfo)
{
	int iNum = (ELTimer_GetMSec() / 67) % WAYPOINT_IMAGE_COUNT;
	if (!rkInfo.m_bRepeat)
	{
		iNum = rkInfo.m_iLastRender;
		int iCurrentTime = (ELTimer_GetMSec() / 67) % WAYPOINT_IMAGE_COUNT;
		if (rkInfo.m_iLastRenderTime != iCurrentTime)
		{
			rkInfo.m_iLastRenderTime = iCurrentTime;
			rkInfo.m_iRenderTime++;
			if (++rkInfo.m_iLastRender >= WAYPOINT_IMAGE_COUNT)
				rkInfo.m_iLastRender = 0;
		}
	}
...
void CPythonMiniMap::AddSignalPoint(float fX, float fY, const std::string& stText, bool bRepeat)
{
	...
	AddWayPoint(TYPE_WAYPOINT, g_id, fX, fY, stText, 0, bRepeat);
