...
class CPythonMiniMap : public CScreen, public CSingleton<CPythonMiniMap>
{
	...
	public:
		...
		// WayPoint
		void AddWayPoint(BYTE byType, DWORD dwID, float fX, float fY, std::string strText, DWORD dwChrVID=0, bool bRepeat=true);
		void RemoveWayPoint(DWORD dwID);

		// SignalPoint
		void AddSignalPoint(float fX, float fY, const std::string& stText = "", bool bRepeat = true);
	...
	protected:
		...
		void __RenderWayPointMark(int ixCenter, int iyCenter, TAtlasMarkInfo& rkInfo);
