...
class CPythonSystem : public CSingleton<CPythonSystem>
{
	...
	public:
...
#ifdef HIDE_NPC_OPTION
		void							SetHideNPC(int category, bool show);
		bool							IsHiddenNPC(int category);
#endif
		...
		// New
		bool							ReadFromRegistry(char* szPIDName, char* szPath, char* szBuf, bool bLocalMachine);
		void							WriteToRegistry(char* szPIDName, char* szPath, char* szContent, bool bLocalMachine);
		const char*						GetMD5(const char* szString);
		const char*						GetHWID();
		const char*						GetHWIDHash();
