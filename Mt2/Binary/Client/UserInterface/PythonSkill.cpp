...
bool CPythonSkill::RegisterSkillTable(const char * c_szFileName)
{
	...
	for (DWORD i = 0; i < textFileLoader.GetLineCount(); ++i)
	{
		...
		{
			TraceError("CPythonSkill::RegisterSkillTable(%s) - Strange Token Count [Line:%d / TokenCount:%d / TABLE_TOKEN_TYPE_MAX_NUM:%d] \n", c_szFileName, i, TokenVector.size(), TABLE_TOKEN_TYPE_MAX_NUM);
			continue;
		}
...
void CPythonSkill::__RegisterGradeIconImage(TSkillData & rData, const char * c_szHeader, const char * c_szImageName)
{
	for (int j = 0; j < SKILL_GRADE_COUNT; ++j)
	{
		...
		_snprintf(szCount, sizeof(szCount), "_%02d", j==3 ? (j-1)+1 : j+1); // P-image = G-image
#else
...
CPythonSkill::CPythonSkill()
{
	...
	m_SkillNeedWeaponIndexMap.insert(std::map<std::string, DWORD>::value_type("ARROW", SKILL_NEED_WEAPON_ARROW | SKILL_NEED_WEAPON_QUIVER));
...
float CPythonSkill::SSkillData::ProcessFormula(CPoly * pPoly, float fSkillLevel, int iMinMaxType, BYTE realLevel)
{
	if (pPoly->Analyze())
	{
		for (DWORD i = 0; i < pPoly->GetVarCount(); ++i)
		{
			...
			else if (!strcmp("sklv", c_szVarName))
				fState = float(realLevel);
			else
			{
				int iState;
...
const char * CPythonSkill::SSkillData::GetAffectDescription(DWORD dwIndex, float fSkillLevel, BYTE realLevel)
{
	...
	// OVERWRITE_SKILLPROTO_POLY
	float fMinValue = ProcessFormula(&minPoly, fSkillLevel, VALUE_TYPE_FREE, realLevel);
	float fMaxValue = ProcessFormula(&maxPoly, fSkillLevel, VALUE_TYPE_FREE, realLevel);
...
PyObject * skillGetSkillDescription(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillDescription - Failed to find skill by %d", iSkillIndex);
...
PyObject * skillGetSkillType(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillType - Failed to find skill by %d", iSkillIndex);
...
PyObject * skillGetSkillConditionDescriptionCount(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillConditionDescriptionCount - Failed to find skill by %d", iSkillIndex);
		return Py_BuildValue("i", 0);
...
PyObject * skillGetSkillConditionDescription(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillConditionDescription() - Failed to find skill by %d", iSkillIndex);
		return Py_BuildValue("s", "");
...
PyObject * skillGetSkillAffectDescriptionCount(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillAffectDescriptionCount - Failed to find skill by %d", iSkillIndex);
		return Py_BuildValue("i", 0);
...
PyObject * skillGetSkillAffectDescription(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (PyTuple_Size(poArgs) > 3)
		if (!PyTuple_GetByte(poArgs, 3, &skillLevel))
			return Py_BadArgument();

	CPythonSkill::SSkillData * c_pSkillData;
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillAffectDescription - Failed to find skill by %d", iSkillIndex);
		return Py_BuildValue("i", 0);
		// return Py_BuildException("skill.GetSkillAffectDescription - Failed to find skill by %d", iSkillIndex);
	}

	return Py_BuildValue("s", c_pSkillData->GetAffectDescription(iAffectIndex, fSkillPoint, skillLevel));
}
...
PyObject * skillGetSkillCoolTime(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillCoolTime - Failed to find skill by %d", iSkillIndex);
		return Py_BuildValue("i", 0);
...
PyObject * skillGetSkillNeedSP(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillNeedSP Failed to find skill by %d", iSkillIndex);
		return Py_BuildValue("i", 0);
...
PyObject * skillGetSkillContinuationSP(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillContinuationSP - Failed to find skill by %d", iSkillIndex);
		return Py_BuildValue("i", 0);
...
PyObject * skillGetSkillMaxLevel(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillMaxLevel - Failed to find skill by %d", iSkillIndex);
		return Py_BuildValue("i", 0);
...
PyObject * skillGetSkillLevelUpPoint(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillLevelUpPoint - Failed to find skill by %d", iSkillIndex);
		return Py_BuildValue("i", 0);
...
PyObject * skillGetSkillLevelLimit(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		TraceError("skill.GetSkillLevelLimit - Failed to find skill by %d", iSkillIndex);
		return Py_BuildValue("i", 0);
...
PyObject * skillCanUseSkill(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (!CPythonSkill::Instance().GetSkillData(iSkillIndex, &c_pSkillData))
	{
		...
		TraceError("skill.CanUseSkill - Failed to find skill by %d", iSkillIndex);
		return Py_BuildValue("i", 0);
	}
...
PyObject * skillGetNewAffectData(PyObject * poSelf, PyObject * poArgs)
{
	...
	float fMinValue = pSkillData->ProcessFormula(&minPoly, fSkillLevel, CPythonSkill::VALUE_TYPE_MIN, skillLevel);
	float fMaxValue = pSkillData->ProcessFormula(&maxPoly, fSkillLevel, CPythonSkill::VALUE_TYPE_MAX, skillLevel);
...
PyObject * skillResetCooltime(PyObject * poSelf, PyObject * poArgs)
{
	int iSkillIndex;
	if (!PyTuple_GetInteger(poArgs, 0, &iSkillIndex))
		return Py_BadArgument();

	CPythonPlayer::Instance().ResetCooltime(iSkillIndex);
	return Py_BuildNone();
}
#endif
...
#ifdef ENABLE_RUNE_SYSTEM
		{ "ResetCooltime",							skillResetCooltime,							METH_VARARGS },
#endif
