...
char pc_name[255], user_name[255], pc_hash[32*2+1];
...
PyObject * packGet(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (pcExt) // 확장자가 있고
	{
			...
			(stricmp(pcExt, ".txt") == 0))
		{
			...
			// TraceError("FILE: %s", strFileName);
			if (!PyTuple_GetString(poArgs, 1, &pFileName)) // Security for anti dump
...
void initpack()
{
	...
	{
		{ "Exists",		packExist,		METH_VARARGS },
		{ "Gets",		packGet,		METH_VARARGS },
		{ NULL, NULL },		
	};
	
	Py_InitModule("timer", s_methods);
}