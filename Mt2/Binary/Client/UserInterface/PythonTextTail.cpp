...
#ifdef ENABLE_EXYGO_FPS_BOOST
	concurrency::parallel_for_each(m_CharacterTextTailList.begin(), m_CharacterTextTailList.end(), [&](TTextTail * pTextTail)
#else
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
#endif
	{
#ifndef ENABLE_EXYGO_FPS_BOOST
...
void CPythonTextTail::UpdateTextTail(TTextTail * pTextTail)
{
	...
	rpyGraphic.ProjectPosition(c_rv3Position.x,
							   ...
							   &pTextTail->y,
							   &pTextTail->z,
							   &pTextTail->pOwner->GetScaleMatrixNew());
...
void CPythonTextTail::ShowAllTextTail()
{
	...
	for (itor = m_CharacterTextTailMap.begin(); itor != m_CharacterTextTailMap.end(); ++itor)
	{
		...
		if (pTextTail->fDistanceFromPlayer < 3500.0f)
			ShowCharacterTextTail(itor->first, true);
	}
...
#ifdef ITEM_TEXTAIL_OPTION
void CPythonTextTail::ShowItemsTextTail()
{
	TTextTailMap::iterator itor;
	for (itor = m_ItemTextTailMap.begin(); itor != m_ItemTextTailMap.end(); ++itor)
	{
...
void CPythonTextTail::ShowCharacterTextTail(DWORD VirtualID, bool bShowAll)
{
	...
	if (bShowAll)
	{
		if (pInstance->GetRace() >= 30000 && pInstance->GetRace() <= 30008)
			return;

		// if (pInstance->IsNPC() && !pInstance->IsEnemy())
			// return;
	}
...
void CPythonTextTail::SetItemTextTailOwner(DWORD dwVID, const char * c_szName)
{
	...
	if (strlen(c_szName) > 0)
	{
		...
		CPythonCharacterManager & rkChrMgr = CPythonCharacterManager::Instance();
		CInstanceBase* pkInstMain = rkChrMgr.GetMainInstancePtr();
		if (!strcmp(c_szName, pkInstMain->GetName().c_str()))
