...
{
	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	rkNetStream.SendCharacterStatePacket(c_rkState.kPPosSelf, c_rkState.fAdvRotSelf, CInstanceBase::FUNC_SKILL, uArg | (uMotSkill << 8));
}
...
void CPythonPlayerEventHandler::FlushVictimList()
{
	...
	network::CGOutputPacket<network::CGSyncPositionPacket> pack;
	...
	for (unsigned int i = 0; i < uiVictimCount; ++i)
	{
		...
		elem->set_vid(rkVictim.m_dwVID);

		long lX = rkVictim.m_lPixelX, lY = rkVictim.m_lPixelY;
		CPythonBackground::Instance().LocalPositionToGlobalPosition(lX, lY);
		elem->set_x(lX);
		elem->set_y(lY);
	}
