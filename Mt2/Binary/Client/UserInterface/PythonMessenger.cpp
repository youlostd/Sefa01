...
{
	m_TeamNameMap.erase(c_szKey);

	if (m_poMessengerHandler)
		PyCallClassMemberFunc(m_poMessengerHandler, "OnRemoveList", Py_BuildValue("(is)", MESSENGER_GROUP_INDEX_TEAM, c_szKey));
}

void CPythonMessenger::RefreshTeamlerState()
{
	if (m_poMessengerHandler)
		PyCallClassMemberFunc(m_poMessengerHandler, "RefreshTeamState", Py_BuildValue("()"));
}
