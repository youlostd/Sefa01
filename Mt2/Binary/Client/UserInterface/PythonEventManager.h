...
class CPythonEventManager : public CSingleton<CPythonEventManager>
{
	...
	public:
		typedef struct SEventSet
		{
			...
			// State
			bool isLock;
			bool isLocalFinish; // local file reading end
	...
	public:
		...
		bool IsWait(int iIndex, bool bCheckLocal = false);
		...
		void HideQuestLetter(int iQuestIndex);

		void SetFontColor(int iIndex, float r, float g, float b, float a);
		size_t GetTotalLineCount(int iIndex);
