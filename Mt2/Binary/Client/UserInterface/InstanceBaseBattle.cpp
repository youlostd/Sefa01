...
BOOL CInstanceBase::CheckAdvancing()
{
	...
	for(CPythonCharacterManager::CharacterIterator i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd();++i)
	{
		...
		if (rkActorEach.IsShop() || rkActorEach.IsMount() || rkActorEach.IsPet())
			continue;
#ifdef ENABLE_AUCTION
		if (CPythonAuction::instance().is_shop_race(rkActorEach.GetRace()))
			continue;
#endif
#ifdef ENABLE_SAFEZONE_NO_COLLISION
		const TPixelPosition& c_rkPPosCur=m_GraphicThingInstance.NEW_GetCurPixelPositionRef();
		if (rkBG.isAttrOn(c_rkPPosCur.x, c_rkPPosCur.y, CTerrainImpl::ATTRIBUTE_BANPK))
			continue;
#endif

#ifdef ENABLE_FAKEBUFF
		if (rkActorEach.IsFakeBuff())
			continue;
#endif
		
		CPythonBackground& rkBG=CPythonBackground::Instance();
		if (!stricmp(rkBG.GetWarpMapName(), "outdoor/metin2_map_oxevent") && rkActorEach.IsPC())
			continue;
