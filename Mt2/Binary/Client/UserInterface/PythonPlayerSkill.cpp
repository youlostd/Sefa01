...
bool CPythonPlayer::__CheckShortMana(TSkillInstance& rkSkillInst, CPythonSkill::TSkillData& rkSkillData)
{
	...
	{
		if (iNeedSP == 0 && (access("________dev.txt", 0) != -1 || access("dev.txt", 0) != -1))
		{
...
DWORD CPythonPlayer::__GetSkillTargetRange(CInstanceBase *rkInstMain, CPythonSkill::TSkillData& rkSkillData)
#else
DWORD CPythonPlayer::__GetSkillTargetRange(CPythonSkill::TSkillData& rkSkillData)
#endif
{
	DWORD targetRange = rkSkillData.GetTargetRange();

#ifdef BOW_RANGE_ONLY_FOR_NINJA
	if (rkInstMain && rkInstMain->GetWeaponType() == CItemData::WEAPON_BOW)
	{
		targetRange += GetStatus(POINT_BOW_DISTANCE) * 100;
	}
#endif
...
{
#ifdef BOW_RANGE_ONLY_FOR_NINJA
	DWORD dwSkillTargetRange = __GetSkillTargetRange(&rkInstMain, rkSkillData);
#else
...
#ifdef BLOCK_DISPEL_ON_MOBS

	if (!rkInstTarget.IsPC() && rkSkillData.dwSkillIndex == 66)
	{
		PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", Py_BuildValue("(is)", GetMainCharacterIndex(), "NEED_TARGET"));
		return false;
	}
		...
		pSkillData->IsAutoSearchTarget())
	{
		...
		if (pkInstTarget)
		{
			if (pSkillData->IsOnlyForAlliance())
			{
				...
				{
#ifdef BOW_RANGE_ONLY_FOR_NINJA
					DWORD dwSkillRange = __GetSkillTargetRange(pkInstMain, *pSkillData);
#else
...
#ifdef BOW_RANGE_ONLY_FOR_NINJA
	DWORD dwRange = __GetSkillTargetRange(pkInstMain, *pSkillData);
#else
...
void CPythonPlayer::ResetCooltime(DWORD dwSkillVnum)
{
	for (size_t i = 0; i < sizeof(m_playerStatus.aSkill); ++i)
		if (m_playerStatus.aSkill[i].dwIndex == dwSkillVnum)
		{
			m_playerStatus.aSkill[i].fLastUsedTime = 0;
			PyCallClassMemberFunc(m_ppyGameWindow, "RunUseSkillEvent", Py_BuildValue("(if)", i, 0.0f));
			return;
		}
}
#endif
...
{
	auto c_pPlayerItemData = CPythonPlayer::Instance().GetItemData(TItemPos(INVENTORY, c_Equipment_Arrow));
	CItemData * pItemData;

	if (CItemManager::Instance().GetItemDataPointer(GetItemIndex(TItemPos(INVENTORY, c_Equipment_Arrow)), &pItemData) && c_pPlayerItemData)
	if (CItemData::ITEM_TYPE_WEAPON == pItemData->GetType())
	if (CItemData::WEAPON_ARROW == pItemData->GetSubType() || (CItemData::WEAPON_QUIVER == pItemData->GetSubType() && c_pPlayerItemData->sockets(1) > 0))
	{
		return true;
	}
