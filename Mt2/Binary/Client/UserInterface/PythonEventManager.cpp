...
const long c_lNormal_Waiting_Time = 5;
...
int CPythonEventManager::RegisterEventSet(const char * c_szFileName)
{
	...
	if (!pEventSet->ScriptGroup.Create(strEventString, true))
	{
...
FORCEINLINE bool __IsReturnChar(char c)
{
	static const char sc_acReturnChars[] = {
		',',
		'.',
		' ',
		';',
	...
	static const int sc_iReturnCharsLen = sizeof(sc_acReturnChars) / sizeof(sc_acReturnChars[0]);

	for (int i = 0; i < sc_iReturnCharsLen; ++i)
	{
		if (c == sc_acReturnChars[i])
			return true;
	}
...
int __FindFirstReturnChar(const char* c_pszString, int iStringLength = -1)
{
	if (iStringLength == -1)
		iStringLength = strlen(c_pszString);

	const char acReturnChars[] = {
		',',
		'.',
		'!',
		'?',
	...
	const int iReturnCharsLen = sizeof(acReturnChars) / sizeof(acReturnChars[0]);

	for (int i = iStringLength - 1 - 1; i > 0; --i)
	{
		for (int j = 0; j < iReturnCharsLen; ++j)
		{
			if (c_pszString[i] == acReturnChars[j])
				return i;
		}
	}
...
void CPythonEventManager::ProcessEventSet(TEventSet * pEventSet)
{
	...
	switch (iEventType)
	{
		case EVENT_TYPE_LETTER:
		{
			...
			std::string stOldValue = pEventSet->strCurrentLine;

			if (c_rstValue.length() == 1 && stOldValue.length() == 0 && c_rstValue[0] == ' ')
				break;

			// check length of next word
			if (stOldValue.length() && __IsReturnChar(stOldValue[stOldValue.length() - 1]))
			{
				std::string stNextWord = c_rstValue;
				
				script::TCmd CurrentScriptCommand;
				while (pEventSet->ScriptGroup.GetNextCmd(CurrentScriptCommand) && !__IsReturnChar(stNextWord[stNextWord.length() - 1]))
				{
					int pCurrentEventPosition;
					int iCurrentEventType;
					if (!GetScriptEventIndex(CurrentScriptCommand.name.c_str(), &pCurrentEventPosition, &iCurrentEventType))
						return;

					if (iCurrentEventType != EVENT_TYPE_LETTER)
						break;

					stNextWord.append(GetArgumentString("value", CurrentScriptCommand.argList));
				}

				pEventSet->strCurrentLine.append(stNextWord);
				pEventSet->pCurrentTextLine->SetValueString(pEventSet->strCurrentLine);
				pEventSet->pCurrentTextLine->Update();

				int iTextWidth, iTextHeight;
				pEventSet->pCurrentTextLine->GetTextSize(&iTextWidth, &iTextHeight);

				pEventSet->strCurrentLine = stOldValue;
				if (iTextWidth > pEventSet->iWidth)
				{
					pEventSet->pCurrentTextLine->SetValueString(pEventSet->strCurrentLine);
		...
		case EVENT_TYPE_QUEST_BUTTON:
		{
			...
			int cat_idx = atoi(GetArgument("cat_idx", ScriptCommand.argList));
			const char * name = GetArgument("name", ScriptCommand.argList);

			// 퀘스트 UI 리뉴얼이 되면 해결 되므로 일단 용혼석만 땜빵 by chrislee
			if (!strcmp(name, "조각난 용혼석?....")) 
			{
				PyCallClassMemberFunc(m_poInterface, "BINARY_RecvQuest", Py_BuildValue("(iisss)", idx, cat_idx, name, "highlight", ""));
			}
			else {
				if (c_rstFile.empty())
				{
					PyCallClassMemberFunc(m_poInterface, "RecvQuest", Py_BuildValue("(iis)", idx, cat_idx, name));
				}
				else
				{
					PyCallClassMemberFunc(m_poInterface, "BINARY_RecvQuest", 
						Py_BuildValue("(iisss)", idx, cat_idx, name, c_rstType.c_str(), c_rstFile.c_str()));
				}
			}
			break;
		}
...
bool CPythonEventManager::IsWait(int iIndex, bool bCheckLocal)
{
	if (!CheckEventSetIndex(iIndex))
		return false;
	...
	if (bCheckLocal && pEventSet->isLocalFinish)
		return false;
...
void CPythonEventManager::HideQuestLetter(int iQuestIndex)
{
	PyCallClassMemberFunc(m_poInterface, "BINARY_ClearQuest", Py_BuildValue("(i)", iQuestIndex));
}

void CPythonEventManager::SetFontColor(int iIndex, float r, float g, float b, float a)
{
	if (!CheckEventSetIndex(iIndex))
		return;

	TEventSet * line = m_EventSetVector[iIndex];
	for (auto it = line->ScriptTextLineList.begin(); it != line->ScriptTextLineList.end(); ++it)
		it->pInstance->SetColor(r, g, b, a);
}

size_t CPythonEventManager::GetTotalLineCount(int iIndex)
{
	if (!CheckEventSetIndex(iIndex))
		return 0;
	...
	return m_EventSetVector[iIndex]->ScriptTextLineList.size();
}
	...
	: m_poInterface(0), m_strLeftTimeString("남은 시간 : %d초")
{
	...
	EventTypeMap["TITLEIMAGE"]=EVENT_TYPE_TITLE_IMAGE;
