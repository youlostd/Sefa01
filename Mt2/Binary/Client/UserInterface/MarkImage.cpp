...
bool CGuildMarkImage::Save(const char* c_szFileName) 
{
	...
	if (!ilSave(IL_PNG, (const ILstring)c_szFileName))
		return false;
...
bool CGuildMarkImage::SaveBlockFromCompressedData(DWORD posBlock, const BYTE * pbComp, DWORD dwCompSize)
{
	...
	int lzoDecompressResult = lzo1x_decompress_safe((const unsigned char*)pbComp, dwCompSize, (BYTE *)apxBuf, (lzo_uint*)&sizeBuf, CLZO::Instance().GetWorkMemory());
	if (LZO_E_OK != lzoDecompressResult)
	{
		sys_err("CGuildMarkImage::CopyBlockFromCompressedData: cannot decompress, compressed size = %u [errorID: %d]", dwCompSize, lzoDecompressResult);
		return false;
	}
...
DWORD CGuildMarkImage::GetEmptyPosition()
{
	...
	for (DWORD row = 0; row < MARK_ROW_COUNT; ++row)
	{
		...
		{
			GetData(col * SGuildMarkBlock::WIDTH, row * SGuildMarkBlock::HEIGHT, SGuildMarkBlock::WIDTH, SGuildMarkBlock::HEIGHT, kMark.m_apxBuf);
...
void SGuildMarkBlock::Compress(const Pixel * pxBuf)
{
	...
	if (LZO_E_OK != lzo1x_999_compress((const BYTE *) pxBuf,
		sizeof(Pixel) * SGuildMarkBlock::SIZE, (unsigned char*) m_abCompBuf,
