...
PyObject * chatGetWhisperLineCount(PyObject* poSelf, PyObject* poArgs)
{
	char * szName;
	if (!PyTuple_GetString(poArgs, 0, &szName))
		return Py_BuildException();

	return Py_BuildValue("i", CPythonChat::Instance().GetWhisperLineCount(szName));
}
...
PyObject * chatGetLinkFromHyperlink(PyObject * poSelf, PyObject * poArgs)
{
	...
	if (0 == results[0].compare("item"))
	{
		...
		if (CItemManager::Instance().GetItemDataPointer(htoi(results[1].c_str()), &pItemData))
		{
			...
			if (htoi(results[1].c_str()) == 50300 && htoi(results[3].c_str()))
			{
				CPythonSkill::SSkillData* c_pSkillData;
				if (CPythonSkill::Instance().GetSkillData(htoi(results[3].c_str()), &c_pSkillData))
...
void initChat()
{
	static PyMethodDef s_methods[] = 
	{
		...
		{ "GetWhisperLineCount",	chatGetWhisperLineCount,			METH_VARARGS },
...
#endif
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_CHAT",				CPythonChat::WHISPER_TYPE_NORMAL);
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_NOT_LOGIN",			CPythonChat::WHISPER_TYPE_NOT_LOGIN);
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_NOT_EXIST",			CPythonChat::WHISPER_TYPE_NOT_EXIST);
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_TARGET_BLOCKED",	CPythonChat::WHISPER_TYPE_TARGET_BLOCKED);
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_SENDER_BLOCKED",	CPythonChat::WHISPER_TYPE_SENDER_BLOCKED);
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_NOAUTH",			CPythonChat::WHISPER_TYPE_NOAUTH);
	PyModule_AddIntConstant(poModule, "WHISPER_TYPE_ERROR",				CPythonChat::WHISPER_TYPE_ERROR);
