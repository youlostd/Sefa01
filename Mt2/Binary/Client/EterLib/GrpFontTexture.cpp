...
bool CGraphicFontTexture::Create(const char* c_szFontName, int fontSize, bool bItalic, bool bIsBold)
{
	Destroy();
...
{
	TFontInfoData kData;
	kData.wCodePage = codePage;
	kData.iFontSize = m_fontSize;
	kData.bIsItalic = m_bItalic;
	kData.bIsBold = m_bIsBold;

	HFONT hFont = NULL;
	TFontMap::iterator i = m_fontMap.find(kData);
	...
	else
	{
		...
		logFont.lfWeight			= m_bIsBold ? FW_BOLD : FW_NORMAL;
		...
		m_fontMap.insert(TFontMap::value_type(kData, hFont));
	}
...
#ifdef ENABLE_IDIOTIC_TEXT_FIX
	if (keyValue == L'P')
	{
		static DWORD s_dwFixCounter = 0;
		if (s_dwFixCounter < 15)
		{
			++s_dwFixCounter;
...
CGraphicFontTexture::TCharacterInfomation* CGraphicFontTexture::UpdateCharacterInfomation(TCharacterKey code)
{
	...
	if (g_iUseNewFont && !strcmp(m_fontName, "tahoma-12"))
		rNewCharInfo.advance += 1;
