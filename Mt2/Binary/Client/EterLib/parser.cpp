...
bool Group::Create(const std::string & stSource, bool bFromLocal)
{
	m_cmdList.clear();
    ...
    while (str_pos < str_len)
    {
		...
		else if (word_len == 1)
		{
			...
			if ((bFromLocal && '[' == cur) || (!bFromLocal && '{' == cur))
			{
				++str_pos;

				const char* box_begin = str_base + str_pos;
				const char* box_end = LocaleString_FindChar(box_begin, str_len - str_pos, bFromLocal ? ']' : '}');
				if (!box_end)
				{
...
bool Group::GetCmd(TCmd & cmd)
{
	...
	m_cmdListIterator = m_cmdList.begin();
    return true;
}

bool Group::GetNextCmd(TCmd & cmd)
{
	if (m_cmdListIterator == m_cmdList.end())
		return false;

	cmd = *(m_cmdListIterator++);
	return true;
}
