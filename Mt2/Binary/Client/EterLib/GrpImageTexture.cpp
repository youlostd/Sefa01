...
bool CGraphicImageTexture::CreateFromPXArray(BYTE* pxBuffer, DWORD dwImageWidth, DWORD dwImageHeight, D3DFORMAT d3dfmt)
{
	assert(ms_lpd3dDevice != NULL);
	assert(m_lpd3dTexture == NULL);
	...
	if (FAILED(D3DXCreateTexture(
		...
		&m_lpd3dTexture)))
	{
		TraceError("CreateFromPXArray: Cannot create texture");
		return false;
	}

	m_width = dwImageWidth;
	m_height = dwImageHeight;

	D3DLOCKED_RECT lockedRect;

	if (FAILED(m_lpd3dTexture->LockRect(0, &lockedRect, NULL, 0)))
	{
		TraceError("CreateFromPXArray: Cannot lock image");
		return false;
	}

	BYTE* fromPtr = pxBuffer;
	BYTE* destPtr = (BYTE*)lockedRect.pBits;
	for (int y = 0; y < dwImageHeight; ++y)
	{
		memcpy(destPtr, fromPtr, dwImageWidth * 4);
		fromPtr += dwImageWidth * 4;
		destPtr += lockedRect.Pitch;
	}
