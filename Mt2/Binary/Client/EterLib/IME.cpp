...
bool CIME::Initialize(HWND hWnd)
{
	...
	ms_uOutputCodePage = GetDefaultCodePage();
...
void CIME::PasteTextFromClipBoard()
{
	...
	if (!buffer || !wcslen(buffer) || wcslen(buffer) > 500)
		return;
	std::wstring wstrClipboard = buffer;
	GlobalUnlock(handle);
	CloseClipboard();
	...
	for (int i = 0; i < wstrClipboard.length(); ++i) {
		TraceError("Clipboard[%d] => %wc (%u)", i + 1, wstrClipboard[i], wstrClipboard[i]);
	}
#endif

	boost::replace_all(wstrClipboard, L"|", L"||");
	InsertString(wstrClipboard.c_str(), wstrClipboard.length());
	if(ms_pEvent)
		ms_pEvent->OnUpdate();
}
...
/*---------------------------------------------------------------------------*/ /* Private */ 
void CIME::InsertString(const wchar_t* wString, int iSize)
{
...
LRESULT CIME::WMChar(HWND /*hWnd*/, UINT /*uiMsg*/, WPARAM wParam, LPARAM lParam, int iLastWMKeyDown)
{
	wchar_t c = (wchar_t)(wParam & 0xffff);
	...
	default:
		...
		// first idx = last key down
		// second idx = wm_char index
		// third idx = real index that replaces the second index
		const wchar_t wcReplaceTable[][3] = {
			{ 0, 253, L'ı' },
			{ 0, 240, L'ğ' },
			{ 0, 208, L'Ğ' },
			{ 0, 254, L'ş' },
			...
			{ 0, 221, L'İ' },
			{ 76, 179, 322 }, // 'ł'
			{ 75, 179, 322 }, // 'ł' in hungarian keyboard lol
			{ 69, 234, 281 }, // 'ę'
			{ 65, 185, 261 }, // 'ą'
			{ 83, 156, 347 }, // 'ś'
			{ 90, 191, 380 }, // 'ż'
		...
		for (int i = 0; i < sizeof(wcReplaceTable) / sizeof(wcReplaceTable[0]); ++i)
		{
			if ((wcReplaceTable[i][0] == 0 || wcReplaceTable[i][0] == iLastWMKeyDown) && wcReplaceTable[i][1] == c)
			{
				c = wcReplaceTable[i][2];
				break;
			}
		}
