...
{ 
  ...
  jpeg::buffer = (JOCTET*)malloc(OUTBUFFER_SIZE);
  if(!jpeg::buffer) {
      perror("malloc");
      printf("Out of memory!\n");
      exit(1);
  }
  dmgr->next_output_byte = jpeg::buffer;
...
{ 
  struct jpeg_destination_mgr*dmgr = 
      (struct jpeg_destination_mgr*)(cinfo->dest);
  if(jpeg::fi)
    fwrite(jpeg::buffer, OUTBUFFER_SIZE, 1, jpeg::fi);
  dmgr->next_output_byte = jpeg::buffer;
  dmgr->free_in_buffer = OUTBUFFER_SIZE;
  return 1;
}
...
{ struct jpeg_destination_mgr*dmgr = 
      (struct jpeg_destination_mgr*)(cinfo->dest);
  if(jpeg::fi)
    fwrite(jpeg::buffer, OUTBUFFER_SIZE-dmgr->free_in_buffer, 1, jpeg::fi);
  free(jpeg::buffer);
  jpeg::buffer = 0;
...
{ 
  struct jpeg_destination_mgr*dmgr = 
      (struct jpeg_destination_mgr*)(cinfo->dest);
  dmgr->next_output_byte = jpeg::dest;
  dmgr->free_in_buffer = jpeg::destlen;
}
...
{ 
  struct jpeg_destination_mgr*dmgr = 
      (struct jpeg_destination_mgr*)(cinfo->dest);
  jpeg::len = jpeg::destlen - dmgr->free_in_buffer;
  dmgr->free_in_buffer = 0;
}
...
void mem_init_source (j_decompress_ptr cinfo)
{
    ...
    mgr->next_input_byte = jpeg::data;
    mgr->bytes_in_buffer = jpeg::size;
    //printf("init %d\n", jpeg::size - mgr->bytes_in_buffer);
}
...
{
    struct jpeg_source_mgr* mgr = cinfo->src;
    printf("skip %d +%d\n", jpeg::size - mgr->bytes_in_buffer, num_bytes);
    if(num_bytes<=0)
	return;
