...
void CScreen::ProjectPosition(float x, float y, float z, float * pfX, float * pfY, float * pfZ, const D3DXMATRIX * pScaling)
{
	D3DXMATRIX tempRotation;
	D3DXMatrixIdentity(&tempRotation);
	D3DXMatrixRotationZ(&tempRotation, D3DXToRadian(0.0f));

	D3DXMATRIX temp;
	D3DXMatrixMultiply(&temp, pScaling, &tempRotation);

	D3DXVECTOR3 Input(x, y, z);
	D3DXVECTOR3 Output;
//	D3DXVec3Project(&Output, &Input, &ms_Viewport, &ms_matProj, &ms_matView, &temp);
	D3DXVec3Project(&Output, &Input, &ms_Viewport, &ms_matProj, &ms_matView, &ms_matWorld);
