...
#if defined(ENABLE_MOB_SCALING) || defined(ENABLE_ACCE_COSTUME_SYSTEM)
	m_v3ScaleNew.x = m_v3ScaleNew.y = m_v3ScaleNew.z = 0.0f;

	D3DXMatrixIdentity(&m_matScale);
#endif

#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	m_v3ScalePos.x = m_v3ScalePos.y = m_v3ScalePos.z = 0.0f;
	m_bIsAcceTransform = false;
...
void CGraphicObjectInstance::Transform()
{
	...
	D3DXMATRIX temp;
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	D3DXMatrixMultiply(&temp, m_bIsAcceTransform ? &m_matPosition : &m_matScale, &m_mRotation);
#else
	D3DXMatrixMultiply(&temp, &m_matScale, &m_mRotation);
#endif
...
#ifdef ENABLE_ACCE_COSTUME_SYSTEM
	D3DXMatrixMultiply(&temp, &m_matPosition, &m_matScale);

	D3DXMATRIX temp2;
	D3DXMatrixMultiply(&temp2, &temp, &m_mRotation);

	m_matAbsoluteTrans = temp2;
	m_matAbsoluteTrans._41 = m_v3ScalePos.x + m_v3Position.x + m_matAbsoluteTrans._41;
	m_matAbsoluteTrans._42 = m_v3ScalePos.y + m_v3Position.y + m_matAbsoluteTrans._42;
	m_matAbsoluteTrans._43 = m_v3ScalePos.z + m_v3Position.z + m_matAbsoluteTrans._43;
#endif
#else
...
#if defined(ENABLE_MOB_SCALING) || defined(ENABLE_ACCE_COSTUME_SYSTEM)
const D3DXVECTOR3 & CGraphicObjectInstance::GetScaleNew() const
{
	return m_v3ScaleNew;
}

const D3DXMATRIX & CGraphicObjectInstance::GetScaleMatrixNew() const
{
	return m_matScale;
}
#endif
...
{
	m_bIsAcceTransform = true;

	m_v3ScaleNew = newscale;
	D3DXMatrixScaling(&m_matScale, m_v3ScaleNew.x, m_v3ScaleNew.y, m_v3ScaleNew.z);
}
