...
void CParticleSystemInstance::OnSetDataPointer(CEffectElementBase * pElement)
{
	...
	CParticleProperty *prop = new CParticleProperty(*m_pData->GetParticlePropertyPointer());
	m_pParticleProperty = prop;
	m_vecParticlePropriety.push_back(prop);
#else
...
#ifdef CHANGE_SKILL_COLOR
	for (auto &prop : m_vecParticlePropriety)
		delete prop;

	m_vecParticlePropriety.clear();
#endif
}
