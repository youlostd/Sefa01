...
bool CPythonLauncher::Create(const char* c_szProgramName)
{
	...
	PyModule_AddIntConstant(builtins, "true", 1);
	PyModule_AddIntConstant(builtins, "false", 0);
	PyModule_AddIntConstant(builtins, "test_server", IsFile("________dev.txt") || IsFile("dev.txt"));
#ifdef ENABLE_RUNE_SYSTEM
	PyModule_AddIntConstant(builtins, "RUNE_ENABLED", 1);
#else
	PyModule_AddIntConstant(builtins, "RUNE_ENABLED", 0);
#endif

#ifdef ELONIA
	PyModule_AddIntConstant(builtins, "__SERVER__", 2);
#else
	PyModule_AddIntConstant(builtins, "__SERVER__", 1);
#endif

	PyModule_AddStringConstant(builtins, "UI", "d:/ymir work/ui/");
	PyModule_AddStringConstant(builtins, "ui", "d:/ymir work/ui/");
	PyModule_AddIntConstant(builtins, "ITEM_ATTRIBUTEINFO", 0); // FAKE -> M2BOB DETECTION
...
bool CPythonLauncher::RunFile(const char* c_szFileName)
{
	...
	
	{
		...
		if (dwBufSize == 0)
		{
			TraceError("cannot run file %s [no size]", c_szFileName);
			return false;
		}
