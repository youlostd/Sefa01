...
void CSlotWindow::GetSlotPosition(int iIndex, int* pLeft, int* pTop)
{
	TSlot * pSlot;
	if (GetSlotPointer(iIndex, &pSlot))
	{
		*pLeft = pSlot->ixPosition;
		*pTop = pSlot->iyPosition;
	}
	else
	{
		*pLeft = -1;
...
#ifndef ENABLE_MARK_NEW_ITEM_SYSTEM
...
void CSlotWindow::OnRender()
{
	...
	for (itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
	{
		...
		{
#ifdef ENABLE_MARK_NEW_ITEM_SYSTEM
			int iIndex = __GetSlotActiveEffectIndex(rSlot);
			if (m_apSlotActiveEffect[iIndex])
			{
				int ix = m_rect.left + rSlot.ixPosition;
				int iy = m_rect.top + rSlot.iyPosition;
				m_apSlotActiveEffect[iIndex]->SetPosition(ix, iy);
				m_apSlotActiveEffect[iIndex]->SetDiffuseColor(
					rSlot.sDiffuseColor.r,
					rSlot.sDiffuseColor.g,
					rSlot.sDiffuseColor.b,
					rSlot.sDiffuseColor.a);
				m_apSlotActiveEffect[iIndex]->Render();
			}
#else
			if (m_pSlotActiveEffect)
			{
...
void CSlotWindow::SetUnusableSlot(DWORD dwSlotNumber, BOOL bFlag)
{
	TSlot* pSlot;
	if (GetSlotPointer(dwSlotNumber, &pSlot))
	{
...
void CSlotWindow::SetUnusableSlotWorld(DWORD dwSlotNumber, BOOL bFlag)
{
	TSlot* pSlot;
	if (GetSlotPointer(dwSlotNumber, &pSlot))
	{
...
void CSlotWindow::SetDiffuseColor(DWORD dwIndex, BYTE colorType)
{
	TSlot * pSlot;
	if (!GetSlotPointer(dwIndex, &pSlot))
		return;
	...
	case SLOT_COLOR_TYPE_WHITE:
		...
		pSlot->sDiffuseColor.a = 0.5f;
		break;
	case SLOT_COLOR_TYPE_DARK_RED:
		pSlot->sDiffuseColor.r = 0.360784f;
		pSlot->sDiffuseColor.g = 0.105882f;
		pSlot->sDiffuseColor.b = 0.0862f;
		pSlot->sDiffuseColor.a = 0.5f;
		break;
	default:
		pSlot->sDiffuseColor.r = 1.0f;
...
#ifdef ENABLE_MARK_NEW_ITEM_SYSTEM
int CSlotWindow::__GetSlotActiveEffectIndex(const TSlot& c_rSlot) const
{
	if (c_rSlot.isItem)
		return MINMAX(0, ((int)c_rSlot.byyPlacedItemSize) - 1, 3 - 1);
	else
		return MINMAX(0, c_rSlot.iyCellSize / c_rSlot.ixCellSize - 1, 3 - 1);
}
#endif
...
{
	CGraphicImage * pImage = (CGraphicImage *)CResourceManager::Instance().GetResourcePointer(c_szFileName);
	// SetSlotNew(dwIndex, dwVirtualNumber, byWidth, byHeight, 1.0f, false, pImage, diffuseColor, NULL, diffuseColor, 0, 0);
...
bool CSlotWindow::IsSlotCovered(TSlot *ppSlot)
{
	int slotPosX = ppSlot->ixPosition;
	int slotPosY = ppSlot->iyPosition;
	...
	for (TSlotListIterator itor = m_SlotList.begin(); itor != m_SlotList.end(); ++itor)
	{
		...
		if (rSlot.isItem)
		{
			if (slotPosX == rSlot.ixPosition)
			{
				iyCellSize = max(rSlot.iyCellSize, int(rSlot.byyPlacedItemSize * ITEM_HEIGHT));

				if (slotPosY >= rSlot.iyPosition && slotPosY < (rSlot.iyPosition + iyCellSize))
				{
					return true;
				}
			}
...
bool CSlotWindow::IsEmptySlot(DWORD slotIndex)
{
	TSlot * pSlot = nullptr;
	if (!GetSlotPointer(slotIndex, &pSlot))
		return false;

	return IsSlotCovered(pSlot) == 0;
}
