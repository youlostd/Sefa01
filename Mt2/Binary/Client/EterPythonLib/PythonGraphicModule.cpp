...
PyObject* grpGetAlphaFromColor(PyObject* poSelf, PyObject* poArgs)
{
	int iColor;
	if (!PyTuple_GetInteger(poArgs, 0, &iColor))
		return Py_BuildException();

	BYTE* abColors = (BYTE*)&iColor;
	return Py_BuildValue("f", (float)abColors[3] / 255.0f);
}
...
PyObject* grpDestoryBigTextBar(PyObject* poSelf, PyObject* poArgs)
{
	int iHandle;
	if (!PyTuple_GetInteger(poArgs, 0, &iHandle))
		return Py_BuildException();
...
void initgrp()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetAlphaFromColor",			grpGetAlphaFromColor,			METH_VARARGS },
		...
		{ "DestoryBigTextBar",			grpDestoryBigTextBar,			METH_VARARGS },
