...
void sys_log(unsigned int bit, const char *format, ...)
{
	...
	if (log_file_sys)
	{
		...
		time_s[strlen(time_s) - 1] = '\0';
		long calcTime;
		if (tv.tv_usec > lastSysLog)
			calcTime = tv.tv_usec - lastSysLog;
		else
			calcTime = 1000000 - lastSysLog + tv.tv_usec;
		fprintf(log_file_sys->fp, "%-15.15s.%d\t[%d]\t :: ", time_s + 4, tv.tv_usec, calcTime);
		lastSysLog = tv.tv_usec;
...
// #ifndef __WIN32__
		// snprintf(system_cmd, 128, "cd %s && tar czf %s.%02d.tgz %s.%02d && rm -f %s.%02d &", dir, logfile->filename, logfile->last_hour, logfile->filename, logfile->last_hour, logfile->filename, logfile->last_hour);
		// system(system_cmd);
// #endif
