...
inline void mb_code(const DWORD sz, const DWORD sy, const DWORD *master, DWORD *dest, int lenght)
{
...
int mb_encrypt(DWORD *dest, const DWORD *src, const DWORD key, int size)
{
	int		i;
	int		resize;
	...
	for (i = 0; i < resize >> 3; i++, dest += 2, src += 2)
		mb_code(*(src + 1), *src, master, dest, 5);
