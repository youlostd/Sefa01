...
char *timediff_str(int time)
{
	...
	if (days > 0)
		len += snprintf(szBuf + len, sizeof(szBuf) - len, "%d days ", days);
	time -= days * 60 * 60 * 24;

	int hours = time / 60 / 60;
	if (hours > 0)
		len += snprintf(szBuf + len, sizeof(szBuf) - len, "%d hours ", hours);
	time -= hours * 60 * 60;

	int minutes = time / 60;
	if (minutes > 0)
		len += snprintf(szBuf + len, sizeof(szBuf) - len, "%d minutes ", minutes);
	time -= minutes * 60;

	int seconds = time;
	if (seconds > 0)
		len += snprintf(szBuf + len, sizeof(szBuf) - len, "%d seconds ", seconds);
