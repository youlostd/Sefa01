		...
		case 0:
			{
				...
				nextUsingTime = random_number(BlueDragon_GetSkillFactor(3, "Skill0", "period", "min"), BlueDragon_GetSkillFactor(3, "Skill0", "period", "max"));
			}
			break;
		...
		case 1:
			{
				...
				nextUsingTime = random_number(BlueDragon_GetSkillFactor(3, "Skill1", "period", "min"), BlueDragon_GetSkillFactor(3, "Skill1", "period", "max"));
			}
			break;
		...
		case 2:
			{
				...
				nextUsingTime = random_number(BlueDragon_GetSkillFactor(3, "Skill2", "period", "min"), BlueDragon_GetSkillFactor(3, "Skill2", "period", "max"));
...
int BlueDragon_Damage (LPCHARACTER me, LPCHARACTER pAttacker, int dam)
{
	...
	if (true == pAttacker->IsMonster() && 2493 == pAttacker->GetMobTable().vnum())
	{
		for (int i=1 ; i <= 4 ; ++i)
		{
	...
	if (true == me->IsMonster() && 2493 == me->GetMobTable().vnum())
	{
		for (int i=1 ; i <= 4 ; ++i)
		{
	...
	if (true == me->IsStone() && 0 != pAttacker->GetMountVnum())
	{
		for (int i=1 ; i <= 4 ; ++i)
		{
			if (me->GetMobTable().vnum() == BlueDragon_GetIndexFactor("DragonStone", i, "vnum"))
			{
