...
CWarMap::CWarMap(long lMapIndex, const TGuildWarInfo & r_info, TWarMapInfo * pkWarMapInfo, DWORD dwGuildID1, DWORD dwGuildID2)
{
	...
	thecore_memcpy(&m_WarInfo, &r_info, sizeof(TGuildWarInfo));
...
void CWarMap::IncMember(LPCHARACTER ch)
{
	...
	bool isWarMember = gid != 0 && (m_TeamData[0].dwID == gid || m_TeamData[1].dwID == gid);
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(ch, "관전 모드로 길드전에 참가하셨습니다."));
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(ch, "자신을 선택하시면 밖으로 나갈 수 있는 <관람 종료> 버튼이 나옵니다."));
	}
...
namespace
{
	struct FNotice
	{
		...
		void operator() (LPCHARACTER ch)
		{
			ch->ChatPacket(CHAT_TYPE_NOTICE, "%s", LC_TEXT(ch, m_psz));
		}
...
void CWarMap::SendWarPacket(LPDESC d)
{
	...
	pack->set_guild_self(m_TeamData[0].dwID);
	pack->set_guild_opponent(m_TeamData[1].dwID);
	pack->set_type(CGuildManager::instance().TouchGuild(m_TeamData[0].dwID)->GetGuildWarType(m_TeamData[1].dwID));
	pack->set_war_state(CGuildManager::instance().TouchGuild(m_TeamData[0].dwID)->GetGuildWarState(m_TeamData[1].dwID));
...
void CWarMap::SendScorePacket(BYTE bIdx, LPDESC d)
{
	...
	p->set_gain_guild_id(m_TeamData[bIdx].dwID);
	p->set_opponent_guild_id(m_TeamData[bIdx ? 0 : 1].dwID);
	p->set_point(m_TeamData[bIdx].iScore);

	if (d)
		d->Packet(p);
	else
		std::for_each(m_set_pkChr.begin(), m_set_pkChr.end(), [&p](LPCHARACTER ch) {
			ch->GetDesc()->Packet(p);
		});
}
...
bool CWarMapManager::LoadWarMapInfo(const char * c_pszFileName)
{
	...
	k->lMapIndex = GUILD_WAR_MAP_INDEX;
	k->posStart[0].x = 48 * 100 + 486400;
	k->posStart[0].y = 52 * 100 + 307200;
	k->posStart[1].x = 183 * 100 + 486400;
	k->posStart[1].y = 206 * 100 + 307200;
	k->posStart[2].x = 141 * 100 + 486400;
	k->posStart[2].y = 117 * 100 + 307200;
	...
	k->lMapIndex = GUILD_FLAG_WAR_MAP_INDEX;
	k->posStart[0].x = 68 * 100 + 512000;
	k->posStart[0].y = 69 * 100 + 307200;
	k->posStart[1].x = 171 * 100 + 512000;
	k->posStart[1].y = 182 * 100 + 307200;
	k->posStart[2].x = 122 * 100 + 512000;
	k->posStart[2].y = 131 * 100 + 307200;
