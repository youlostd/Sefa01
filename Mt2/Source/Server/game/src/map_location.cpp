...
bool CMapLocation::GetByChannel(BYTE bChannel, int iIndex, long& lAddr, WORD& wPort)
{
	auto it = m_map_address_by_channel.find(std::make_pair(bChannel, iIndex));
	if (it == m_map_address_by_channel.end())
		return false;
...
void CMapLocation::Insert(long lIndex, const char* c_pszHost, WORD wPort, BYTE bChannel)
{
	TLocation loc;
	...
	m_map_address_by_channel[std::make_pair(bChannel, lIndex)] = loc;
	sys_log(0, "MapLocation::Insert : %d %s %d %d", lIndex, c_pszHost, wPort, bChannel);
}
