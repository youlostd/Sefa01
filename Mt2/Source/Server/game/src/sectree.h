...
class SECTREE
{
	public:
		...
		template <class _Func> LPENTITY	find_if (_Func & func) const
		{
			...
			{
				ENTITY_SET::const_iterator it_entity = (*it_tree)->m_set_entity.begin();
	...
	private:
		...
		template <class _Func> void for_each_pc_entity(_Func & func)
		{
			itertype(m_set_entityPC) it = m_set_entityPC.begin();
			for (; it != m_set_entityPC.end(); ++it)
			{
				func(*it);
			}
		}
