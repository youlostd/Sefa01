...
void CMob::AddSkillSplash(int iIndex, DWORD dwTiming, DWORD dwHitDistance)
{
	...
	sys_log(0, "MOB_SPLASH %s idx %d timing %u hit_distance %u",
			m_table.locale_name(LANGUAGE_DEFAULT).c_str(), iIndex, dwTiming, dwHitDistance);
...
bool CMobManager::Initialize(const google::protobuf::RepeatedPtrField<network::TMobTable>& table)
{
	m_map_pkMobByVnum.clear();
	for (int i = 0; i < LANGUAGE_MAX_NUM; ++i)
		m_map_pkMobByName[i].clear();
	...
	for (auto& t : table)
	{
		...
		m_map_pkMobByVnum.insert(std::map<DWORD, CMob*>::value_type(t.vnum(), pkMob));
		for (int i = 0; i < LANGUAGE_MAX_NUM; ++i)
			m_map_pkMobByName[i].insert(std::map<std::string, CMob*>::value_type(t.locale_name(i), pkMob));

		int SkillCount = 0;

		for (int j = 0; j < MOB_SKILL_MAX_NUM; ++j)
		{
			if (pkMob->m_table.skills_size() <= j)
			{
				sys_err("LoadMob [%u] skill %d missing!!", pkMob->m_table.vnum(), j);
				pkMob->m_table.add_skills();
			}

			if (pkMob->m_table.skills(j).vnum())
				++SkillCount;
		}

		sys_log(0, "MOB: #%-5d %-30s LEVEL %u HP %u DEF %u EXP %u DROP_ITEM_VNUM %u SKILL_COUNT %d",
				t.vnum(), t.name().c_str(), t.level(), t.max_hp(), t.def(), t.exp(), t.drop_item_vnum(), SkillCount);

		if (t.type() == CHAR_TYPE_NPC || t.type() == CHAR_TYPE_WARP || t.type() == CHAR_TYPE_GOTO)
			CHARACTER_MANAGER::instance().RegisterRaceNum(t.vnum());
	...
	snprintf(szGroupFileName, sizeof(szGroupGroupFileName),
			"%s/group.txt", Locale_GetBasePath().c_str());
	snprintf(szGroupGroupFileName, sizeof(szGroupGroupFileName),
			"%s/group_group.txt", Locale_GetBasePath().c_str());
...
const CMob * CMobManager::Get(const char * c_pszName, bool bIsAbbrev, int iLanguageID)
{
	if (iLanguageID < 0 || iLanguageID >= LANGUAGE_MAX_NUM)
	{
		for (int i = 0; i < LANGUAGE_MAX_NUM; ++i)
		{
			const CMob * pkMob = Get(c_pszName, bIsAbbrev, i);
			if (pkMob)
				return pkMob;
		}
	...
	if (!bIsAbbrev)
	{
		it = m_map_pkMobByName[iLanguageID].find(c_pszName);

		if (it == m_map_pkMobByName[iLanguageID].end())
			return NULL;
	...
	it = m_map_pkMobByName[iLanguageID].begin();

	while (it != m_map_pkMobByName[iLanguageID].end())
	{
...
bool CMobManager::LoadGroup(const char * c_pszFileName)
{
	...
	for (DWORD i = 0; i < loader.GetChildNodeCount(); ++i)
	{
		...
		sys_log(!test_server, "GROUP: %-5d %s", iVnum, stName.c_str());
		sys_log(!test_server, "			   %s %s", pTok->at(0).c_str(), pTok->at(1).c_str());
