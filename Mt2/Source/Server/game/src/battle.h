...
extern void Item_GetDamage(LPITEM pkItem, int* pdamMin, int* pdamMax);
extern void Item_GetMagicDamage(LPITEM pkItem, int* pmagicDamMin, int* pmagicDamMax);
...
inline void AttackAffect(LPCHARACTER pkAttacker,
		...
		const char* name)
{
	if (pkAttacker->GetPoint(att_point) && (!affect_flag || !pkVictim->IsAffectFlag(affect_flag)))
	{
		if (random_number(1, 100) <= pkAttacker->GetPoint(att_point) && !pkVictim->IsImmune(immune_flag))
		{
...
inline void SkillAttackAffect(LPCHARACTER pkVictim,
		...
		const char* name)
{
	if (success_pct && (!affect_flag || !pkVictim->IsAffectFlag(affect_flag)))
	{
		if (random_number(1, 1000) <= success_pct && !pkVictim->IsImmune(immune_flag))
		{
			pkVictim->AddAffect(affect_idx, affect_point, affect_amount, affect_flag, time, 0, true);
