...
bool SECTREE::InsertEntity(LPENTITY pkEnt)
{
	...
	if (pkCurTree)
	{
		if (pkEnt->IsType(ENTITY_CHARACTER) && ((LPCHARACTER)pkEnt)->IsPC())
			pkCurTree->m_set_entityPC.erase(pkEnt);
