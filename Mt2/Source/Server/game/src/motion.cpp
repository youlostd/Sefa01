...
static float MOB_GetNormalAttackDuration(network::TMobTable* mobTable)
{
	float minDuration = 99.0f;

	const char * folder = mobTable->folder().c_str();
...
static const char* GetMotionFileName(network::TMobTable* mobTable, EPublicMotion motion)
{
	char buf[1024];
	const char * folder = mobTable->folder().c_str();
	...
	if (fp != NULL)
	{
		...
		switch (motion)
		{
			...
			default:
				fclose(fp);
				sys_err("Motion: no process for this motion(%d) vnum(%d)", motion, mobTable->vnum());
				return NULL;
		}
	...
	{
		sys_err("Motion: %s have not motlist.txt vnum(%d) folder(%s)", folder, mobTable->vnum(), mobTable->folder().c_str());
	}

	return NULL;
}

static void LoadMotion(CMotionSet* pMotionSet, network::TMobTable* mob_table, EPublicMotion motion)
{
	...
	{
		M2_DELETE(pMotion);
		sys_err("Motion: Load failed vnum(%d) motion(%d) file(%s)", mob_table->vnum(), motion, cpFileName);
	}
}
...
static void LoadSkillMotion(CMotionSet* pMotionSet, CMob* pMob, EPublicMotion motion)
{
	...
	network::TMobTable* mob_table = &pMob->m_table;

	if (mob_table->skills(idx).vnum() == 0) return;
	...
	{
		if (mob_table->skills(idx).vnum() != 0)
		{
			sys_err("Motion: Skill exist but no motion data for index %d mob %u skill %u file %s",
				   	idx, mob_table->vnum(), mob_table->skills(idx).vnum(), cpFileName);
		}
		M2_DELETE(pMotion);
	}
}
...
bool CMotionManager::Build()
{
	...
	for (int i = 0; i < MAIN_RACE_MAX_NUM; ++i)
	{
		...
		snprintf(sz, sizeof(sz), "%s/claw/attack.msa", c_apszFolderName[i]);
		pkMotionSet->Load(sz, MOTION_MODE_CLAW, MOTION_NORMAL_ATTACK);
		snprintf(sz, sizeof(sz), "%s/claw/combo_01.msa", c_apszFolderName[i]);
		pkMotionSet->Load(sz, MOTION_MODE_CLAW, MOTION_COMBO_ATTACK_1);
		snprintf(sz, sizeof(sz), "%s/claw/combo_02.msa", c_apszFolderName[i]);
		pkMotionSet->Load(sz, MOTION_MODE_CLAW, MOTION_COMBO_ATTACK_2);
		snprintf(sz, sizeof(sz), "%s/claw/combo_03.msa", c_apszFolderName[i]);
		pkMotionSet->Load(sz, MOTION_MODE_CLAW, MOTION_COMBO_ATTACK_3);
		snprintf(sz, sizeof(sz), "%s/claw/combo_04.msa", c_apszFolderName[i]);
		pkMotionSet->Load(sz, MOTION_MODE_CLAW, MOTION_COMBO_ATTACK_4);
#endif
	...
	while (it != CMobManager::instance().end())
	{
		...
		auto t = &pkMob->m_table;

		if ('\0' != t->folder().c_str()[0])
		{
			CMotionSet * pkMotionSet = M2_NEW CMotionSet;
			m_map_pkMotionSet.insert(TContainer::value_type(t->vnum(), pkMotionSet));
			...
			sys_log(0, "mob_normal_attack_duration:%d:%s:%.2f", t->vnum(), t->folder().c_str(), normalAttackDuration);
			m_map_normalAttackDuration.insert(std::map<DWORD, float>::value_type(t->vnum(), normalAttackDuration));
			// END_OF_POLYMORPH_BUG_FIX
		}
	}
...
bool CMotion::LoadMobSkillFromFile(const char * c_pszFileName, CMob* pMob, int iSkillIndex)
{
	...
	for (DWORD i = 0; i < rkTextFileLoader.GetChildNodeCount(); ++i)
	{
		...
		if (0 == strNodeName.compare("motioneventdata"))
		{
			...
			for (DWORD j = 0; j < dwMotionEventDataCount; ++j)
			{
				...
				pMob->AddSkillSplash(iSkillIndex, 100 + DWORD(fStartingTime * 1000), (DWORD)abs(v3Position.y));
	...
	pMob->m_mobSkillInfo[iSkillIndex].dwSkillVnum = pMob->m_table.skills(iSkillIndex).vnum();
	pMob->m_mobSkillInfo[iSkillIndex].bSkillLevel = pMob->m_table.skills(iSkillIndex).level();
	m_isEmpty = false;
	return true;
}
