...
void CGuildMarkImage::GetDiffBlocks(const ::google::protobuf::RepeatedField<uint32_t>& crc_list, std::map<BYTE, const SGuildMarkBlock *> & mapDiffBlocks)
{
	BYTE posBlock = 0;
	DWORD idx = 0;
	...
	for (DWORD row = 0; row < BLOCK_ROW_COUNT; ++row)
		...
		{
			if (crc_list.size() <= idx || m_aakBlock[row][col].m_crc != crc_list[idx])
			{
