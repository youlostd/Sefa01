	...
	for (i = 0; i < num; ++i)
	{
		...
		else if (regen->sx == regen->ex && regen->sy == regen->ey)
		{
			ch = CHARACTER_MANAGER::instance().SpawnMob(regen->vnum,
					...
					regen->z_section,
					false,
					regen->direction == 0 ? random_number(0, 7) * 45 : (regen->direction - 1) * 45);
...
bool regen_do(const char* filename, long lMapIndex, int base_x, int base_y, LPDUNGEON pDungeon, bool bOnce)
{
	...
	while (true)
	{
			...
			tmp.type == REGEN_TYPE_ANYWHERE)
		{
			...
			{
				regen = M2_NEW REGEN;
				thecore_memcpy(regen, &tmp, sizeof(REGEN));
			}
			else
				regen = &tmp;
			...
			if (!bOnce && pDungeon != NULL)
			{
				...
				regen->event = event_create(dungeon_regen_event, info, PASSES_PER_SEC(random_number(0, 16)) + PASSES_PER_SEC(regen->time));
...
time_t get_seconds_since_midnight()
{
	auto stamp = std::time(NULL);
	std::tm* ptm = std::localtime(&stamp);
	return ( ptm->tm_hour * 3600 ) + ( ptm->tm_min * 60 ) + ( ptm->tm_sec );
}

time_t get_global_seconds()
{
	return std::time(NULL);
}

int get_next_spawn_delay(int respawn_time, time_t current_seconds)
{
	int next_spawn_time = 0;

	while(next_spawn_time <= current_seconds)
		next_spawn_time += respawn_time;

	return next_spawn_time - current_seconds;
}

std::string get_time_string(int seconds_to_add)
{
	auto stamp = std::time(NULL) + seconds_to_add;
...
EVENTFUNC(regen_event)
{
	...
	DWORD regen_time = regen->time;
	bool is_important_spawn = ( regen_time >= 3600 );
	...
	return PASSES_PER_SEC(regen_time);
}
	...
	while (true)
	{
			...
			tmp.type == REGEN_TYPE_ANYWHERE)
		{
			...
			if (regen->type == REGEN_TYPE_MOB)
			{
				...
				else if (p->m_table.type() == CHAR_TYPE_NPC || p->m_table.type() == CHAR_TYPE_WARP || p->m_table.type() == CHAR_TYPE_GOTO)
				{
					std::string stNamelist[LANGUAGE_MAX_NUM];
					for (int i = 0; i < LANGUAGE_MAX_NUM; ++i)
						stNamelist[i] = p->m_table.locale_name(i);
