...
class CGuildManager : public singleton<CGuildManager>
{
	public:
		...
		const char*	EscapedGuildName(const char* c_pszGuildName);
		...
		void			ReserveWarAdd(const TGuildWarReserve& data);
		void			ReserveWarDelete(DWORD dwID);
		std::vector<CGuildWarReserveForGame *> & GetReserveWarRef();
		void		ReserveWarBet(DWORD war_id, const std::string& login, DWORD gold, DWORD guild_id);
		...
		void		SendGuildList(LPCHARACTER ch, DWORD pageNumber, BYTE pageType, BYTE empire);
		void		SendSpecificGuildRank(LPCHARACTER ch, const char* searchName, BYTE pageType, BYTE empire);
	...
	private:
		...
		DWORD m_dwLastRankUpdate;
		std::vector<network::TGuildLadderInfo*> m_vecRanking[EMPIRE_MAX_NUM];
