...
bool CPolymorphUtils::PolymorphCharacter(LPCHARACTER pChar, LPITEM pItem, const CMob* pMob)
{
	...
	iPolyPercent = pChar->GetLevel() - pMob->m_table.level() + pItem->GetSocket(2) + (29 + bySkillLevel);

	if (iPolyPercent <= 0)
	{
		pChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(pChar, "µÐ°©¿¡ ½ÇÆÐ ÇÏ¿´½À´Ï´Ù"));
		return false;
	}
	else
	{
		if (random_number(1, 100) > iPolyPercent)
		{
			pChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(pChar, "µÐ°©¿¡ ½ÇÆÐ ÇÏ¿´½À´Ï´Ù"));
			return false;
		}
	...
	pChar->AddAffect(AFFECT_POLYMORPH, POINT_POLYMORPH, pMob->m_table.vnum(), AFF_POLYMORPH, dwDuration, 0, true);

	// º¯½Å º¸³Ê½º = µÐ°© ½ºÅ³ ·¹º§ + µÐ°©¼­ ·¹º§
	dwBonusPercent = bySkillLevel + pItem->GetSocket(2);

	switch (GetBonusType(pMob->m_table.vnum()))
	{
...
bool CPolymorphUtils::UpdateBookPracticeGrade(LPCHARACTER pChar, LPITEM pItem)
{
	...
	if (pItem->GetSocket(1) > 0)
		...
		pChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(pChar, "µÐ°©¼­ ¼ö·ÃÀ» ¸¶Ãº½À´Ï´Ù. ½Å¼±¿¡°Ô Ã£¾Æ°¡¼¼¿ä."));
