...
void SendTargetCreatePacket(LPDESC d, TargetInfo * info)
{
	...
	network::GCOutputPacket<network::GCTargetCreatePacket> pck;

	pck->set_id(info->iID);
	pck->set_type(info->iType);
	pck->set_vid(info->iArg1);
	pck->set_name(info->szTargetDesc);
	d->Packet(pck);
}

void SendTargetUpdatePacket(LPDESC d, int iID, int x, int y)
{
	network::GCOutputPacket<network::GCTargetUpdatePacket> pck;
	pck->set_id(iID);
	pck->set_x(x);
	pck->set_y(y);
	d->Packet(pck);
...
{
	network::GCOutputPacket<network::GCTargetDeletePacket> pck;
	pck->set_id(iID);
	d->Packet(pck);
}
/////////////////////////////////////////////////////////////////////
...
// <Factor> Raplaced direct pointer reference with key searching.
	...
	if (iDist <= 500 && (!strcmp("Shop", info->szTargetName) || !strcmp("TARGET_TYPE_POS", info->szTargetName)))
