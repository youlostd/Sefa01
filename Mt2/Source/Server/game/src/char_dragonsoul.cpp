...
void CHARACTER::DragonSoul_GiveQualification()
{
	if(NULL == FindAffect(AFFECT_DRAGON_SOUL_QUALIFIED))
	{
		...
		AddAffect(AFFECT_DRAGON_SOUL_QUALIFIED, APPLY_NONE, 0, AFF_NONE, INFINITE_AFFECT_DURATION, 0, true, false);
	}
...
bool CHARACTER::DragonSoul_ActivateDeck(int deck_idx)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT(this, "용혼석 상자가 활성화되지 않았습니다."));
		return false;
	}
...
bool CHARACTER::DragonSoul_RefineWindow_Open(LPENTITY pEntity)
{
	...
	network::GCOutputPacket<network::GCDragonSoulRefinePacket> PDS;
	PDS->set_sub_type(DS_SUB_HEADER_OPEN);
	LPDESC d = GetDesc();
...
{
	return true;//NULL != m_pointsInstant.m_pDragonSoulRefineWindowOpener;
}
#endif
