...
LPSECTREE_MAP SECTREE_MANAGER::BuildSectreeFromSetting(TMapSetting & r_setting, TMapConfigMap & r_configMap)
#else
...
#endif
{
	...
	pkMapSectree->m_configMap = r_configMap;
...
#ifdef __MAP_CONFIG__
void SECTREE_MANAGER::LoadConfigFile(const char * c_pszConfigFileName, TMapConfigMap & r_configMap)
{
	r_configMap.clear();

	FILE * fp = fopen(c_pszConfigFileName, "r");

	if (!fp)
	{
		//	sys_err("cannot open file: %s", c_pszConfigFileName);
		return;
	}
	...
	while (fgets(buf, 256, fp))
	{
		...
		r_configMap[str_to_lower(key)] = value;
	}
}
#endif
...
#ifdef __MAP_CONFIG__
TMapConfigMap* SECTREE_MANAGER::GetMapConfig(int iMapIndex)
{
	if (iMapIndex >= 10000)
		iMapIndex /= 10000;

	LPSECTREE_MAP pkMap = GetMap(iMapIndex);
	if (pkMap)
		return &pkMap->m_configMap;

	return NULL;
}

std::string SECTREE_MANAGER::GetMapConfig(int iMapIndex, const std::string& c_rstConfigKey, const char* c_pszDefault)
{
	TMapConfigMap* pConfigMap = GetMapConfig(iMapIndex);
	if (!pConfigMap)
		return c_pszDefault;

	TMapConfigMap::iterator it = pConfigMap->find(str_to_lower(c_rstConfigKey.c_str()));
...
int SECTREE_MANAGER::GetMapConfig_Int(int iMapIndex, const std::string& c_rstConfigKey, int iDefault)
{
	std::string stVal = GetMapConfig(iMapIndex, c_rstConfigKey);
	if (stVal.empty())
		return iDefault;

	return atoi(stVal.c_str());
}

bool SECTREE_MANAGER::GetMapConfig_Bool(int iMapIndex, const std::string& c_rstConfigKey, bool bDefault)
{
	std::string stVal = GetMapConfig(iMapIndex, c_rstConfigKey);
	if (stVal.empty())
		return bDefault;

	if (!strcasecmp(stVal.c_str(), "false"))
		return false;
	if (!strcasecmp(stVal.c_str(), "no"))
		return false;
	if (!strcmp(stVal.c_str(), "0"))
		return false;
...
int SECTREE_MANAGER::Build(const char * c_pszListFileName, const char* c_pszMapBasePath)
{
	...
	while (fgets(buf, 256, fp))
	{
		char* pNextLineChar = strrchr(buf, '\n');
		if (pNextLineChar)
			*pNextLineChar = '\0';
...
#ifdef __MAP_CONFIG__
		TMapConfigMap configMap;

		snprintf(szFilename, sizeof(szFilename), "%s/%s/Config.txt", c_pszMapBasePath, szMapName);

		LoadConfigFile(szFilename, configMap);
#endif
		...
		{
#ifdef __MAP_CONFIG__
			LPSECTREE_MAP pkMapSectree = BuildSectreeFromSetting(setting, configMap);
#else
...
bool SECTREE_MANAGER::GetRandomLocation(long lMapIndex, PIXEL_POSITION & r_pos, DWORD dwCurrentX, DWORD dwCurrentY, int iMaxDistance)
{
	...
	while (it != m_vec_mapRegion.end())
	{
		...
		while (i++ < 100)
		{
			x = random_number(rRegion.sx + 50, rRegion.ex - 50);
			y = random_number(rRegion.sy + 50, rRegion.ey - 50);
...
void SECTREE_MANAGER::SendNPCPosition(LPCHARACTER ch)
{
	...
	network::GCOutputPacket<network::GCNPCListPacket> p;

	itertype(m_mapNPCPosition[lMapIndex]) it;

	for (it = m_mapNPCPosition[lMapIndex].begin(); it != m_mapNPCPosition[lMapIndex].end(); ++it)
	{
		auto np = p->add_positions();
		np->set_type(it->bType);
		np->set_name(it->namelist[ch->GetLanguageID()]);
		np->set_race(it->dwRace);
		np->set_x(it->x);
		np->set_y(it->y);
	}
	...
	if (test_server)
		sys_log(0, "SendNPCPosition size %u", p->ByteSize());
}

void SECTREE_MANAGER::InsertNPCPosition(long lMapIndex, DWORD dwRace, unsigned char bType, std::string* pstNameList, long x, long y)
{
	m_mapNPCPosition[lMapIndex].push_back(npc_info(bType, dwRace, pstNameList, x, y));
}
...
bool SECTREE_MANAGER::ForAttrRegion(long lMapIndex, long lStartX, long lStartY, long lEndX, long lEndY, long lRotate, DWORD dwAttr, EAttrRegionMode mode)
{
	...
	if (!pkMapSectree)
	{
		if (map_allow_find(lMapIndex))
			sys_err("2Cannot find SECTREE_MAP by map index %d", lMapIndex);
...
struct FCountSpecifiedMonster
{
	...
	FCountSpecifiedMonster(DWORD id, bool allowNPC = false)
		: SpecifiedVnum(id), cnt(0), AllowAllNPC(allowNPC)
	{}
	...
	void operator() (LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (pChar->IsStone() || (AllowAllNPC && pChar->IsNPC()))
			{
				if (pChar->GetMobTable().vnum() == SpecifiedVnum)
					cnt++;
			}
		}
	}
...
size_t SECTREE_MANAGER::GetMonsterCountInMap(long lMapIndex, DWORD dwVnum, bool bAllowNPC)
{
	LPSECTREE_MAP sectree = SECTREE_MANAGER::instance().GetMap(lMapIndex);

	if (sectree)
	{
		struct FCountSpecifiedMonster f(dwVnum, bAllowNPC);
