...
bool CPeerBase::Accept(socket_t fd_accept)
{
	...
	OnAccept();
	sys_log(0, "ACCEPT FROM %s:%d", inet_ntoa(peer.sin_addr), peer.sin_port);
	return true;
}
...
void CPeerBase::Packet(network::TDGHeader header, uint32_t handle, const ::google::protobuf::Message* packet)
{
	network::TPacketHeader data;
	data.header = static_cast<uint16_t>(header);

	PacketSend(data, handle, packet);
}

void CPeerBase::PacketSend(network::TPacketHeader& header, uint32_t handle, const ::google::protobuf::Message* data)
{
	auto extra_size = data ? data->ByteSizeLong() : static_cast<size_t>(0);
	header.size = sizeof(header) + extra_size + sizeof(uint32_t);

	std::vector<BYTE> data_array;
	data_array.resize(extra_size);
	if (extra_size && !data->SerializeToArray(&data_array[0], extra_size))
		return;

	PacketSend(&header, sizeof(header));
	PacketSend(&handle, sizeof(uint32_t));
	if (extra_size)
		PacketSend(&data_array[0], extra_size);
}

void CPeerBase::PacketSend(const void * c_pvData, int iSize)
{
	if (!m_outBuffer)
	{
	...
	buffer_write(m_outBuffer, c_pvData, iSize);
