...
int CDBManager::Connect(int iSlot, const char * db_address, const int db_port, const char * db_name, const char * user, const char * pwd)
{
	...
	if (!m_directSQL[iSlot]->Setup(db_address, user, pwd, db_name, "utf8", true, db_port))
	{
		Clear();
		return false;
	}
	...
	if (!m_mainSQL[iSlot]->Setup(db_address, user, pwd, db_name, "utf8", false, db_port))
	{
		Clear();
		return false;
	}
	...
	if (!m_asyncSQL[iSlot]->Setup(db_address, user, pwd, db_name, "utf8", false, db_port))
	{
		Clear();
		return false;
	}
...
SQLMsg * CDBManager::DirectQuery(const char * c_pszQuery, int iSlot)
{
	...
	if (g_test_server)
		sys_err("DirectQuery %s", c_pszQuery);
#endif
...
#ifdef USE_QUERY_LOGGING
	if (g_test_server)
		sys_err("ReturnQuery %s", c_pszQuery);
#endif
...
#ifdef USE_QUERY_LOGGING
	if (g_test_server)
		sys_err("AsyncQuery %s", c_pszQuery);
#endif
	assert(iSlot < SQL_MAX_NUM);
