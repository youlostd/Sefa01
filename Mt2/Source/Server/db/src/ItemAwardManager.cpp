...
void ItemAwardManager::RequestLoad()
{
	...
	snprintf(szQuery, sizeof(szQuery), "SELECT id,login,vnum,count,socket0,socket1,socket2,mall,no_trade,why FROM item_award WHERE taken_time IS NULL and id > %d", g_dwLastCachedItemAwardID);
...
void ItemAwardManager::Load(SQLMsg * pMsg)
{
	...
	for (uint i = 0; i < pMsg->Get()->uiNumRows; ++i)
	{
		...
		str_to_number(kData->bIsGMOwner, row[col++]);
		...
		if (row[col])
		{
			...
			{
				network::DGOutputPacket<network::DGItemAwardInformerPacket> giftData;
				giftData->set_login(kData->szLogin);
				giftData->set_command(command);					//명령어 복사
				giftData->set_vnum(kData->dwVnum);				//아이템 vnum도 복사
				CClientManager::instance().ForwardPacket(giftData);
			}
		}
