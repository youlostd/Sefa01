...
{
	SGuild() : ladder_point(0), gold(0), level(0)
	{
...
typedef struct SGuildWarInfo
{
    ...
    SGuildWarInfo() : pElement(NULL), bWarType(0)
    {
    }
} TGuildWarInfo;
...
class CGuildManager : public singleton<CGuildManager>
{
	...
	bool	WaitStart(DGOutputPacket<DGGuildWarPacket> p);
	...
	//
	void	BootReserveWar();
	bool	ReserveWar(std::unique_ptr<GDGuildWarPacket> p);
	void	SendReserverWarAddPacket(const TGuildWarReserve& t, CPeer* peer = nullptr);
	...
	void GuildWarWin(DWORD GID, BYTE type);
	void GuildWarDraw(DWORD GID, BYTE type);
	void GuildWarLose(DWORD GID, BYTE type);

	void ProcessDraw(DWORD dwGuildID1, DWORD dwGuildID2, BYTE warType);
	void ProcessWinLose(DWORD dwGuildWinner, DWORD dwGuildLoser, BYTE warType);
