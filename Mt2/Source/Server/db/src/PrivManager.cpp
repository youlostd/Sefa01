...
void CPrivManager::AddEmpirePriv(BYTE empire, BYTE type, int value, time_t duration_sec)
{
	...
	{
		sys_err("Invalid empire id %u", empire);
		return;
	}
...
struct FSendChangeGuildPriv
{
	FSendChangeGuildPriv(DWORD guild_id, BYTE type, int value, time_t end_time_sec)
	{
		...
		p->set_log(true);
		// ADD_GUILD_PRIV_TIME
		p->set_end_time_sec(end_time_sec);
		// END_OF_ADD_GUILD_PRIV_TIME
	}
	...
	network::DGOutputPacket<network::DGChangeGuildPrivPacket>p;
};
...
struct FSendChangeEmpirePriv
{
	...
	network::DGOutputPacket<network::DGChangeEmpirePrivPacket>p;
};
...
struct FSendChangeCharPriv
{
	...
	network::DGOutputPacket<network::DGChangeCharacterPrivPacket>p;
};
